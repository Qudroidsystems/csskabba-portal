...
<body>
    <div class="header">
        <h1 class="school-name">{{ $schoolInfo->school_name ?? 'TRINITY COMPREHENSIVE INTERNATIONAL SCHOOL ONDO' }}</h1>
    ...
    <table>
        ...
        <tbody>
            ...
            @foreach($students as $std)
                <tr class="student-row">
                    ...
                    @foreach($studentBillInfo as $bill)
                        @php
                            $billPayment = $studentPayments->where('schoolbillid', $bill->schoolbillid)
                                                            ->where('stid', $std->stid)
                                                            ->first();
        ...
        <tfoot>
            <tr class="text-bold">
                ...
                @foreach($studentBillInfo as $bill)
                    @php
                        $billTotalPaid = $studentPayments->where('schoolbillid', $bill->schoolbillid)->sum('totalAmountPaid') ?? 0;
                        $billTotalOwed = ($students->count() * $bill->amount) - $billTotalPaid;
                    @endphp
                    <th class="cell-amount">{{ number_format($billTotalPaid, 2) }}</th>
                    <th class="cell-amount">{{ number_format($billTotalOwed, 2) }}</th>
                @endforeach
                @php
                    $overallTotalPaid = collect($studentTotals)->sum('totalPaid');
                    $overallTotalBill = $students->count() * $studentBillInfo->sum('amount');
    ...
    <div class="footer">
        ...
        <p>Generated by viteschool | Developed by <a href="https://qudroidsystems.com">Qudroid Systems</a></p>
    </div>
</body>
</html>
