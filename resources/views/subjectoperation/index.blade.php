...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Subject checkboxes — count update
        ...
        <div class="card border-0 shadow-sm mb-3" style="border-radius:12px;overflow:hidden;">
            <div class="card-header d-flex justify-content-between align-items-center py-3 px-4"
                ...
                <div class="d-flex gap-2">
                    ...
                    <span class="badge rounded-pill px-3 py-2" style="background:#EEEDFE;color:#3C3489;">
...
async function doRestore(archiveIds, label) {
    const ok = await Swal.fire({
        ...
        icon : 'question', showCancelButton: true,
        confirmButtonColor: '#28a745', confirmButtonText: 'Yes, restore!'
    });
    if (!ok.isConfirmed) return;
...
async function restoreSingleSnapshot(metaEncoded) {
    ...
    const ok = await Swal.fire({
        ...
        icon : 'question', showCancelButton: true,
        confirmButtonColor: '#28a745', confirmButtonText: 'Yes, restore all!'
    });
    if (!ok.isConfirmed) return;
    ...
    try {
        ...
        const detailRes  = await fetch(ROUTES.getSnapshot + '?' + params.toString(), {
            headers: { 'X-CSRF-TOKEN': CSRF, 'Accept': 'application/json' }
        });
