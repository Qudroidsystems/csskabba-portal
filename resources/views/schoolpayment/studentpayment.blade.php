...
<div class="container-fluid">
    ...
    <div class="loading-overlay" id="loadingOverlay">
        <div class="loading-spinner">
            ...
            <p class="mt-2 mb-0 small">Processing...</p>
        </div>
    </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body p-4">
                ...
                <div class="mb-3">
                    ...
                    <div class="form-text text-muted">Amount will be distributed across selected bills starting from earliest.</div>
                </div>
                <div class="mb-3">
                ...
                <div id="paymentDistribution" class="mt-3" style="display:none;">
                    <label class="form-label">Payment Distribution Preview</label>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ── Read params from URL (set by the GET form submission) ──────────────
    // FIX: Also fall back to PHP-passed variables in case URL params are missing
            ...
            if (result.success) {
                ...
                document.getElementById('paymentContent').innerHTML = `
                    <div class="alert alert-danger">
                        <i class="ri-error-warning-line me-2"></i>${escapeHtml(result.message || 'Failed to load payment data')}
                    </div>`;
            }
        })
        .catch(err => {
            ...
            document.getElementById('paymentContent').innerHTML = `
                <div class="alert alert-danger">
                    <i class="ri-error-warning-line me-2"></i>An error occurred while loading payment data. Please refresh and try again.
                </div>`;
        })
    ...
    function renderPaymentContent(data) {
        ...
        const avatarHtml = student.avatar
            ? `<img src="/storage/images/studentavatar/${escapeHtml(student.avatar)}" alt="${escapeHtml(student.name)}"
        ...
        const billsHtml = bills.map(bill => {
            const cardClass = bill.is_paid
                ? 'paid'
                : (bill.is_partial ? 'partial' : (bill.total_savings > 0 ? 'savings' : 'unpaid'));

            const billJson = escapeAttr(JSON.stringify(bill));

            return `
            <div class="col-xl-4 col-lg-6">
                <div class="bill-card ${cardClass}" data-bill-id="${bill.id}">
                    ...
                    <div class="d-flex align-items-start justify-content-between mb-2 mt-1">
                        ...
                        <div class="ms-2 flex-shrink-0">
                            <input type="checkbox" class="bill-select-checkbox" data-bill-id="${bill.id}"
                                   ${bill.is_paid ? 'disabled' : ''} style="transform:scale(1.2);">
                        </div>
                    </div>
                    ...
                    ${!bill.is_paid
                        ? (bill.has_pending_invoice
                            ? `<button class="btn btn-secondary btn-sm w-100" disabled><i class="ri-lock-line me-1"></i>Invoice Pending</button>`
                            : `<button class="btn btn-primary btn-sm w-100 make-payment-btn" data-bill='${billJson}'><i class="ri-wallet-line me-1"></i>Make Payment</button>`)
        ...
        const paymentRecordsHtml = paymentRecords.length > 0
            ? `<div class="table-responsive">
                <table class="table rec-table w-100 mb-0">
                    <thead><tr><th>#</th><th>Bill</th><th>Bill Amount</th><th>Amount Paid</th><th>Balance</th><th>Method</th><th>Received By</th><th>Date</th><th>Status</th><th>Action</th></tr></thead>
                    <tbody>
                    ${paymentRecords.map((sp, i) => `
                        <tr>
                            ...
                            <td>${sp.recordId
                                ? `<button class="btn btn-sm btn-danger delete-payment" data-record-id="${sp.recordId}"><i class="ri-delete-bin-line"></i></button>`
                ...
                <div class="d-flex align-items-start gap-4 flex-wrap">
                    ...
                    <div class="flex-grow-1">
                        <div class="d-flex align-items-center gap-2 flex-wrap mb-1">
                            ...
                            ${student.student_status === 'Active'
                                ...
                                : `<span class="badge bg-danger-subtle text-danger px-2 py-1" style="font-size:11px">${escapeHtml(student.student_status || 'Unknown')}</span>`}
                            ${student.statusId == 1
                    ...
                    <div class="d-flex gap-2 flex-wrap align-items-start">
                        ...
                        ${paymentRecords.length > 0
                            ? `<a href="{{ url('schoolpayment/invoice') }}/${studentId}/${student.schoolclassId || ''}/${termid}/${sessionid}" class="btn btn-primary btn-sm">
                        ...
                        <button class="btn btn-success btn-sm" id="bulkPaymentBtn"
                                ${bills.filter(b => !b.is_paid && !b.has_pending_invoice).length === 0 ? 'disabled' : ''}>
                ...
                <div>
                    ...
                    <div class="small">
                        ${scholarship.value_type === 'percentage'
                            ? `${scholarship.value}% deduction on applicable fees.`
                            : `₦${fmt(scholarship.value)} fixed deduction per bill.`}
                        ${scholarship.effective_to ? ` Valid until ${new Date(scholarship.effective_to).toLocaleDateString()}.` : ''}
                                       ...
                                       ? `<div class="mt-4 p-3 rounded-3" style="background:linear-gradient(135deg,#f3e8ff,#ede9fe);border:1px solid #ddd6fe">
                                              <div class="d-flex align-items-center gap-2">
                                                  ...
                                                  <div>
                                                      ...
                                                      <span class="fw-bold ms-1" style="color:#7c3aed">₦${fmt(totals.savings)}</span>
        ...
        // Attach all event listeners after DOM is updated
        ...
        if (bulkBtn) {
            bulkBtn.addEventListener('click', () => openBulkPaymentModal(bills));
        }
    }
    ...
    // For use in data-bill='...' attributes — escape single quotes
    ...
    function attachBillSelectionEvents(bills) {
        document.querySelectorAll('.bill-select-checkbox').forEach(cb => {
            cb.addEventListener('change', e => {
                ...
                if (e.target.checked) selectedBillsForBulk.add(billId);
                else selectedBillsForBulk.delete(billId);

                const countEl = document.getElementById('selectedCount');
                if (countEl) countEl.textContent = selectedBillsForBulk.size;

                document.querySelectorAll('.bill-card').forEach(card => {
                    card.classList.toggle('selected', selectedBillsForBulk.has(parseInt(card.dataset.billId)));
                });
            });
        });
    }
    ...
    function attachPaymentButtons() {
        document.querySelectorAll('.make-payment-btn').forEach(btn => {
            btn.addEventListener('click', function () {
                try {
                    // data-bill attribute uses &quot; for double quotes — parse via DOM
                    const raw = this.getAttribute('data-bill').replace(/&quot;/g, '"').replace(/&#39;/g, "'");
                    const billData = JSON.parse(raw);
    ...
    // ── Open individual payment modal ─────────────────────────────────────

    function openPaymentModal(bill) {
        document.getElementById('modal-bill-title').textContent   = bill.title;
        document.getElementById('student_id').value               = studentId;
        document.getElementById('class_id').value                 = bill.class_id || '';  // FIX: now comes from server
    ...
    function openBulkPaymentModal(bills) {
        ...
        if (selectedBills.length === 0) {
            Swal.fire({ icon: 'warning', title: 'No Bills Selected', text: 'Please select at least one bill using the checkboxes.', confirmButtonColor: '#2563eb' });
            return;
        }

        const totalPayable = selectedBills.reduce((s, b) => s + b.balance, 0);
        const totalSavings = selectedBills.reduce((s, b) => s + (b.total_savings || 0), 0);
        ...
        document.getElementById('bulk_payment_amount').oninput = () => {
            let amount    = parseFloat(document.getElementById('bulk_payment_amount').value.replace(/[^0-9.]/g, '')) || 0;
            ...
            for (const bill of selectedBills) {
                ...
                const pay = Math.min(remaining, bill.balance);
                dist.push({ title: bill.title, amount: pay, newBalance: bill.balance - pay });
                remaining -= pay;
            }
    ...
    function submitBulkPayment(selectedBills) {
        ...
        if (paymentAmount > totalPayable) {
            return Swal.fire({ icon: 'warning', title: 'Amount Exceeds Balance', text: `Total outstanding is ₦${fmt(totalPayable)}.`, confirmButtonColor: '#2563eb' });
        }
        if (!paymentMethod) {
            return Swal.fire({ icon: 'warning', title: 'Missing Method', text: 'Please select a payment method.', confirmButtonColor: '#2563eb' });
        }

        showLoading(true);

        // FIX: Use actual class_id from the first selected bill (not hardcoded 1)
        ...
        fetch('{{ route("schoolpayment.bulk-store") }}', {
            ...
            body: JSON.stringify({
                ...
                class_id:       classId,   // FIX: was hardcoded as 1
                term_id:        termid,
    ...
    // ── Individual payment form submit ────────────────────────────────────
    ...
    document.getElementById('paymentForm').addEventListener('submit', function (e) {
        ...
        if (amount > balance) {
            return Swal.fire({ icon: 'warning', title: 'Amount Exceeds Balance', text: `Balance is ₦${fmt(balance)}`, confirmButtonColor: '#2563eb' });
        }
        if (!method) {
            return Swal.fire({ icon: 'warning', title: 'Missing Method', text: 'Select a payment method.', confirmButtonColor: '#2563eb' });
        }
    ...
    document.getElementById('confirmDeleteBtn').addEventListener('click', function () {
        ...
        .catch(() => Swal.fire('Error', 'Failed to delete payment. Please try again.', 'error'))
        .finally(() => {
            self.disabled = false;
