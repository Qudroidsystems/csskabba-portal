...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadPaymentData() {
        ...
        fetch(`{{ route('schoolpayment.getPaymentDetailsAjax') }}?studentId=${studentId}&termid=${termid}&sessionid=${sessionid}`, {
            headers: {
    ...
    function renderPaymentContent(data) {
        ...
        const avatarHtml = student.avatar
            ? `<img src="{{ Storage::url('images/studentavatar/${student.avatar}') }}" alt="${student.name}" class="student-avatar-lg">`
                ...
                <div class="bill-card ${cardClass}" data-bill-id="${bill.id}" data-bill-title="${bill.title.replace(/'/g, "\\'")}">
                    ...
                    <div class="d-flex align-items-start justify-content-between mb-2 mt-1">
                        <div class="flex-grow-1">
                            ...
                            ${bill.description ? `<div class="text-muted" style="font-size:11px">${bill.description}</div>` : ''}
                        </div>
            ...
            <div class="table-responsive">
                <table class="table rec-table w-100 mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>${i + 1}</td>
                            <td><div class="fw-semibold">${sp.title}</div>${sp.description ? `<div class="text-muted small">${sp.description}</div>` : ''}</td>
                            ...
                            <td>${sp.recordId ? `<button class="btn btn-sm btn-danger delete-payment" data-url="{{ route('schoolpayment.deletestudentpayment', '') }}/${sp.recordId}"><i class="ri-delete-bin-line"></i></button>` : '<span class="text-muted small">—</span>'}</td>
                        </tr>
                        `).join('')}
                    </tbody>
                </table>
            ...
            <div class="table-responsive">
                <table class="table rec-table w-100 mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>${i + 1}</td>
                            <td><div class="fw-semibold">${ph.title}</div>${ph.description ? `<div class="text-muted small">${ph.description}</div>` : ''}</td>
                            ...
                            <td><a href="{{ route('schoolpayment.invoice', ['studentId' => $studentId, 'schoolclassid' => $schoolclassId, 'termid' => $termid, 'sessionid' => $sessionid]) }}?historical=1" class="btn btn-sm btn-outline-primary" title="View Invoice"><i class="ri-file-download-line"></i></a></td>
                        </tr>
                        `).join('')}
                    </tbody>
                </table>
        ...
        const contentHtml = `
                ...
                <div class="d-flex align-items-start gap-4 flex-wrap">
                    ...
                    <div class="flex-grow-1">
                        <div class="d-flex align-items-center gap-2 flex-wrap mb-1">
                            <h5 class="mb-0 fw-bold" style="color:var(--pay-primary)">${student.name}</h5>
                    ...
                    <div class="d-flex gap-2 flex-wrap align-items-start">
                        ...
                        ${paymentRecords.length > 0 ?
                            `<a href="{{ route('schoolpayment.invoice', ['studentId' => $studentId, 'schoolclassid' => $schoolclassId, 'termid' => $termid, 'sessionid' => $sessionid]) }}" class="btn btn-primary btn-sm"><i class="ri-file-download-line me-1"></i>Generate Invoice</a>` :
                        ...
                        <a href="{{ route('schoolpayment.statement', ['studentId' => $studentId, 'schoolclassid' => $schoolclassId, 'termid' => $termid, 'sessionid' => $sessionid]) }}" class="btn btn-outline-primary btn-sm"><i class="ri-file-list-line me-1"></i>Statement</a>
            ...
            <div class="benefit-banner schol">
                ...
                <div>
                    <div class="fw-semibold mb-1">Scholarship Active: ${scholarship.title}</div>
            ...
            <div class="benefit-banner disc">
                ...
                <div>
                    ...
                    <div class="small">${discounts.map(d => `<span class="me-3"><strong>${d.title}:</strong> ${d.value_type === 'percentage' ? `${d.value}% off` : `₦${fmt(d.value)} off`}</span>`).join('')}</div>
                </div>
            </div>
            ` : ''}
    ...
    function attachDeleteButtons() {
        document.querySelectorAll('.delete-payment').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                confirmDeletePayment(deleteUrl);
            });
        });
    }
    ...
    function openPaymentModal(bill) {
        ...
        document.getElementById('class_id').value = bill.class_id || document.querySelector('.student-card')?.dataset.classId;
    ...
    function submitBulkPayment(selectedBills) {
        ...
        fetch('{{ route("schoolpayment.bulk-store") }}', {
            ...
            body: JSON.stringify({
                ...
                class_id: document.querySelector('.student-card')?.dataset.classId || 1,
                term_id: termid,
    ...
    function confirmDeletePayment(deleteUrl) {
        ...
        document.getElementById('confirmDeleteBtn').onclick = () => {
            ...
            .catch(() => Swal.fire({ icon: 'error', title: 'Error', text: 'Failed to delete payment.' }))
            .finally(() => showLoading(false));
        };
    }
