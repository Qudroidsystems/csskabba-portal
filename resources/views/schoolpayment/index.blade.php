...
<div class="container-fluid">
    ...
    {{-- ══════════════════════════════════════════════════════
         MASS STUDENT MODAL (Full Implementation with Working Photos)
    ══════════════════════════════════════════════════════ --}}
...
/* Helper function to generate avatar data URL from initials */
.avatar-placeholder-print {
    width: 70px;
    height: 70px;
    border-radius: 50%;
    display: inline-flex;
...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Helper to generate avatar data URL from initials (for print fallback)
        ...
        // Helper to get student photo URL - same logic as schoolpayment/index.blade.php
        function getStudentPhotoUrl(student) {
            ...
            if (student.picture && student.picture != 'unnamed.jpg' && student.picture != '') {
                return '{{ asset("storage/images/student_avatars") }}/' + student.picture;
            }
            return null;
        }
        ...
        // Print with photos - 9 per page (UPDATED with WORKING PHOTOS)
        document.getElementById('printResults')?.addEventListener('click', () => {
            ...
            // Merge created and reset credentials, preserving photo info
            const allCreds = [
            ...
            // For each credential, also try to find matching student from selectedStudents to get photo
            const credsWithPhotos = allCreds.map(cred => {
                // Try to find matching student in selectedStudents by id or admission number
                const matchingStudent = selectedStudents.find(s =>
                    s.id == cred.student_id ||
                    s.admissionNo == cred.admissionNo ||
                    s.id == cred.id
                );
                let photoUrl = cred.photo_url || cred.photo;
                let initials = cred.initials || 'ST';
                let studentName = cred.name;

                if (matchingStudent) {
                    photoUrl = photoUrl || matchingStudent.photo_url;
                    initials = matchingStudent.initials || getStudentInitials(matchingStudent);
                    studentName = matchingStudent.name || cred.name;
                }
            ...
            const slips = credsWithPhotos.map(s => {
                ...
                // Build photo HTML - using the same logic as schoolpayment
                let photoHtml = '';
                if (s.photo_url && s.photo_url !== '' && s.photo_url !== 'null') {
                    photoHtml = `<div class="slip-photo"><img src="${s.photo_url}" alt="Photo" onerror="this.onerror=null;this.parentElement.innerHTML='<div class=\\'slip-photo-fallback\\'>${s.initials || 'ST'}</div>'"></div>`;
                } else {
                    // Create colored initials circle as fallback (like schoolpayment avatar-placeholder)
                    const initialsVal = (s.initials || 'ST').substring(0, 2);
                    photoHtml = `<div class="slip-photo"><div class="slip-photo-fallback avatar-placeholder-print" style="background: linear-gradient(135deg, #667eea, #764ba2);">${escHtml(initialsVal)}</div></div>`;
                }
            ...
            <style>
                * { margin:0; padding:0; box-sizing:border-box; }
                ...
                .slip-photo-fallback { font-size:28px; font-weight:700; display:flex; align-items:center; justify-content:center; width:100%; height:100%; background:linear-gradient(135deg, #667eea, #764ba2); color:white; }
                .avatar-placeholder-print { background: linear-gradient(135deg, #667eea, #764ba2); color: white; }
                .slip-info { flex:1; }
