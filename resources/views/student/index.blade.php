    ...
    <div class="page-content">
        <div class="container-fluid">
            ...
            <style>
                ...
                .sm-hero::before { content:''; position:absolute; top:-70px; right:-70px; width:240px; height:240px; background:rgba(255,255,255,.06); border-radius:50%; }
                .sm-hero::after  { content:''; position:absolute; bottom:-40px; left:180px; width:150px; height:150px; background:rgba(255,255,255,.04); border-radius:50%; }
                ...
                .sm-stat {
                    ...
                    border-radius:var(--sm-radius); padding:20px 22px;
                ...
                .sm-stat::before { content:''; position:absolute; top:0; left:0; right:0; height:3px; background:var(--sm-stat-color, var(--sm-accent)); }
                .sm-stat-icon { width:52px; height:52px; border-radius:12px; display:flex; align-items:center; justify-content:center; font-size:22px; color:#fff; margin-bottom:14px; background:var(--sm-stat-color, var(--sm-accent)); box-shadow:0 4px 12px rgba(0,0,0,.12); }
                ...
                /* MAIN PANEL */
                .sm-panel { background:#fff; border:1px solid var(--sm-border); border-radius:var(--sm-radius); overflow:hidden; box-shadow:var(--sm-shadow); }
                .sm-panel-header { padding:16px 20px; border-bottom:1px solid var(--sm-border); display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:10px; }
                .sm-panel-title { font-size:15px; font-weight:700; color:var(--sm-primary); display:flex; align-items:center; gap:8px; }

                /* FILTER BAR */
                .sm-filter { padding:16px 20px; border-bottom:1px solid var(--sm-border); background:#fafbfc; }
                ...
                .sm-search input { padding-left:38px; padding-right:36px; border-radius:10px; border:1.5px solid var(--sm-border); height:42px; font-size:13px; transition:border .2s, box-shadow .2s; width:100%; }
                .sm-search input:focus { border-color:var(--sm-accent); box-shadow:0 0 0 3px rgba(37,99,235,.1); outline:none; }
                .sm-search .clear-btn { position:absolute; right:6px; top:50%; transform:translateY(-50%); background:transparent; border:none; color:#9ca3af; font-size:15px; padding:4px 7px; cursor:pointer; display:none; }
                .sm-search .clear-btn:hover { color:var(--sm-danger); }
                .sm-filter select { border-radius:10px; border:1.5px solid var(--sm-border); height:42px; font-size:13px; padding:0 12px; width:100%; transition:border .2s; background:#fff; }
                .sm-filter select:focus { border-color:var(--sm-accent); outline:none; }

                /* TABLE */
                .sm-table thead th { background:var(--sm-primary); color:#fff; padding:13px 14px; font-weight:600; font-size:12px; text-transform:uppercase; letter-spacing:.04em; border:none; white-space:nowrap; }
                ...
                /* AVATAR */
                .sm-avatar { width:44px; height:44px; border-radius:12px; object-fit:cover; border:2px solid var(--sm-border); cursor:pointer; transition:transform .2s, box-shadow .2s; }
                .sm-avatar:hover { transform:scale(1.08); box-shadow:0 4px 14px rgba(0,0,0,.15); }
                .sm-avatar-init { width:44px; height:44px; border-radius:12px; background:linear-gradient(135deg,#667eea,#764ba2); display:inline-flex; align-items:center; justify-content:center; font-size:16px; font-weight:700; color:#fff; border:2px solid var(--sm-border); cursor:pointer; transition:transform .2s, box-shadow .2s; }
                ...
                /* ACTION BUTTONS */
                .act-btn { width:32px; height:32px; border-radius:8px; border:none; display:inline-flex; align-items:center; justify-content:center; font-size:14px; cursor:pointer; transition:all .15s; }
                ...
                /* STUDENT CARD */
                .stu-card { background:#fff; border:1px solid var(--sm-border); border-radius:var(--sm-radius); overflow:hidden; transition:transform .2s, box-shadow .2s, border-color .2s; height:100%; position:relative; }
                .stu-card:hover { transform:translateY(-4px); box-shadow:var(--sm-shadow-lg); border-color:var(--sm-accent); }
                .stu-card-header { background:linear-gradient(135deg,#1e3a5f 0%,#2563eb 60%,#4f46e5 100%); padding:18px 16px 14px; position:relative; min-height:110px; }
                .stu-card-avatar { position:absolute; top:14px; right:14px; width:68px; height:68px; border-radius:12px; border:3px solid rgba(255,255,255,.9); overflow:hidden; background:#fff; box-shadow:0 4px 12px rgba(0,0,0,.2); cursor:pointer; transition:transform .2s; }
                .stu-card-avatar:hover { transform:scale(1.06); }
                .stu-card-avatar img { width:100%; height:100%; object-fit:cover; }
                .stu-card-avatar-init { width:100%; height:100%; display:flex; align-items:center; justify-content:center; font-size:24px; font-weight:700; color:#667eea; background:linear-gradient(135deg,#f3f4f6,#e5e7eb); }
                ...
                .stu-card-actions button { flex:1; padding:8px 4px; border-radius:8px; border:none; font-size:12px; font-weight:600; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:4px; transition:all .15s; }
                ...
                .img-zoomed { max-width:90vw; max-height:72vh; border-radius:16px; border:4px solid #fff; box-shadow:0 24px 64px rgba(0,0,0,.4); object-fit:contain; cursor:pointer; animation:zoomIn .25s ease; }
                @keyframes zoomIn { from{opacity:0;transform:scale(.85)} to{opacity:1;transform:scale(1)} }
                .img-zoom-modal .btn-close { position:fixed; top:20px; right:28px; background:rgba(0,0,0,.7); border-radius:50%; padding:12px; filter:brightness(0) invert(1); opacity:1; z-index:9999; }
                .img-zoom-modal .btn-close:hover { background:rgba(0,0,0,.9); transform:scale(1.1); }
                .zoom-name   { color:#fff; margin-top:18px; font-size:17px; font-weight:700; background:rgba(0,0,0,.5); padding:7px 22px; border-radius:40px; }
                ...
                /* MODAL HEADERS */
                .modal-hdr { background:linear-gradient(135deg,#1e3a5f,#2563eb 55%,#4f46e5 100%); padding:22px 28px; color:#fff; border:none; position:relative; overflow:hidden; }
                ...
                .draggable-item { background:#fff; border:1.5px solid var(--sm-border) !important; border-radius:10px !important; padding:8px 12px !important; cursor:default; user-select:none; transition:box-shadow .15s; position:relative; display:inline-flex; align-items:center; gap:6px; white-space:nowrap; }
                .draggable-item:hover { box-shadow:0 2px 8px rgba(0,0,0,.08); }
                .drag-handle { cursor:grab !important; color:#9ca3af; padding:4px 6px; margin-right:2px; border-radius:6px; display:inline-flex; align-items:center; transition:color .15s, background .15s; font-size:16px; }
                ...
                .bsc-avatar { width:56px; height:56px; border-radius:50%; margin:0 auto 10px; background:linear-gradient(135deg,#667eea,#764ba2); display:flex; align-items:center; justify-content:center; font-size:22px; font-weight:700; color:#fff; border:3px solid #fff; box-shadow:0 2px 8px rgba(0,0,0,.12); }
                ...
                <div class="sm-panel-header">
                    ...
                    <div class="d-flex flex-wrap align-items-center gap-2">
                        <div class="btn-group view-toggle" role="group">
                            <button type="button" class="btn btn-outline-primary active" id="tableViewBtn"><i class="fas fa-table me-1"></i>Table</button>
                            <button type="button" class="btn btn-outline-primary" id="cardViewBtn"><i class="fas fa-th-large me-1"></i>Cards</button>
                        </div>
                        @can('Delete student')
                        <div class="dropdown">
                            <button class="btn btn-light btn-sm dropdown-toggle" type="button" id="bulkActionsDropdown" data-bs-toggle="dropdown" aria-expanded="false" disabled>
                                <i class="fas fa-cog me-1"></i>Actions
                            </button>
                            <ul class="dropdown-menu shadow-sm border-0 rounded-3 p-2">
                                <li><a class="dropdown-item rounded-2" href="javascript:void(0);" id="deleteMultipleBtn"><i class="fas fa-trash text-danger me-2"></i>Delete Selected</a></li>
                                <li><a class="dropdown-item rounded-2" href="javascript:void(0);" id="updateCurrentTermBtn"><i class="fas fa-calendar-alt text-primary me-2"></i>Update Current Term</a></li>
                            </ul>
                        </div>
                        @endcan
                        @can('Create student')
                        <button type="button" class="btn-pg" data-bs-toggle="modal" data-bs-target="#addStudentModal"><i class="fas fa-user-plus me-1"></i>Add Student</button>
                        @endcan
                        <button type="button" class="btn btn-outline-success btn-sm" data-bs-toggle="modal" data-bs-target="#printStudentReportModal"><i class="fas fa-file-export me-1"></i>Export</button>
                    </div>
                </div>
                ...
                <div class="sm-filter">
                    <div class="row g-2 align-items-end">
                        ...
                        <div class="col-md-1 col-6">
                            <button class="btn btn-primary w-100" id="filterBtn" style="height:42px;border-radius:10px;"><i class="fas fa-filter"></i></button>
                        </div>
                        <div class="col-md-1 col-6">
                            <button class="btn btn-outline-secondary w-100" id="resetFiltersBtn" style="height:42px;border-radius:10px;"><i class="fas fa-redo-alt"></i></button>
                        </div>
                        <div class="col-md-1 col-6">
                            <button class="btn btn-warning w-100" id="bulkStatusBtn" title="Bulk Update Status" style="height:42px;border-radius:10px;font-size:12px;"><i class="fas fa-sync-alt"></i></button>
                        </div>
                        <div class="col-md-1 col-6">
                            <button class="btn btn-info w-100 text-white" id="manageTermBtn" title="Manage Term" style="height:42px;border-radius:10px;font-size:12px;"><i class="fas fa-calendar-alt"></i></button>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn-pg" id="confirmUpdateCurrentTerm"><i class="fas fa-save me-1"></i>Register / Update</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
                <div class="modal-content">
                        ...
                        <form id="printReportForm">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="alert alert-info py-2 px-3 mb-2" style="font-size:12px;">
                                    <i class="fas fa-info-circle me-1"></i>Drag the <i class="fas fa-grip-vertical"></i> handle to reorder. Check to include in report.
                                </div>
                                <div id="columnsContainer">
                                    ...
                                    @foreach($availableColumns as $key => $label)
                                        <div class="draggable-item" data-column="{{ $key }}">
                                            ...
                                            <input class="form-check-input column-checkbox m-0" type="checkbox" name="columns[]" value="{{ $key }}" id="col_{{ $key }}"
                                                {{ in_array($key, ['admissionNo','lastname','firstname','class','gender']) ? 'checked' : '' }}>
                                            <label class="form-check-label mb-0" for="col_{{ $key }}" style="font-size:12px;font-weight:600;cursor:pointer;">{{ $label }}</label>
                                        </div>
                                    @endforeach
                                </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-success rounded-3" id="generateReportBtn"><i class="ri-printer-line me-1"></i>Generate &amp; Download</button>
                    </div>
                </div>
            </div>
        </div>
                                        ...
                                        <div class="card-body">
                                            <div class="mb-3 text-center">
                                                <div class="upload-area border border-2 border-dashed border-primary rounded p-3">
                                                    <img id="addStudentAvatar" src="[via.placeholder.com](https://via.placeholder.com/120x120/2563eb/ffffff?text=Photo)" alt="Avatar Preview" class="rounded-circle mb-2" style="width:100px;height:100px;object-fit:cover;border:4px solid #2563eb;"/>
                                                    <div>
        ...
        <!-- EDIT STUDENT MODAL — scrollable fixed -->
                            ...
                            <div class="row g-4">
                                    ...
                                    <div class="card border-0 shadow-sm h-100">
                                        ...
                                        <div class="card-body">
                                            <div class="mb-3">
                                                <label class="form-label fw-semibold">Admission Number Mode <span class="text-danger">*</span></label>
                                                <div class="d-flex gap-3">
                                                    <div class="form-check"><input class="form-check-input" type="radio" name="admissionMode" id="editAdmissionAuto" value="auto" onchange="toggleAdmissionInput('edit')"><label class="form-check-label" for="editAdmissionAuto">Auto</label></div>
                                                    <div class="form-check"><input class="form-check-input" type="radio" name="admissionMode" id="editAdmissionManual" value="manual" onchange="toggleAdmissionInput('edit')"><label class="form-check-label" for="editAdmissionManual">Manual</label></div>
                                                </div>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label fw-semibold">Admission Number <span class="text-danger">*</span></label>
                                ...
                                <div class="col-md-6">
                                    <div class="card border-0 shadow-sm h-100">
                                        ...
                                        <div class="card-body">
                                            <div class="mb-3 text-center">
                                                <img id="editStudentAvatar" src="{{ asset('theme/layouts/assets/media/avatars/blank.png') }}" alt="Avatar" class="rounded-circle mb-2" style="width:100px;height:100px;object-fit:cover;border:4px solid #0dcaf0;cursor:pointer;" onclick="document.getElementById('editAvatar').click()"/>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-footer bg-light">
                        ...
                        <button type="button" class="btn btn-primary rounded-3" onclick="editStudentFromView()"><i class="fas fa-edit me-1"></i>Edit Student</button>
                         <button type="button" class="btn btn-success rounded-3" onclick="printStudentProfile()"><i class="fas fa-print me-1"></i>Print Profile</button>
                    </div>
                </div>
            </div>
        </div>
...
<script src="[cdn.jsdelivr.net](https://cdn.jsdelivr.net/npm/sortablejs@latest/Sortable.min.js)"></script>
...
(function () {
    ...
    const U = {
        ...
        hideLoading: () => {
            ...
            const has = AppState.pagination.data?.length > 0;
            if (has) {
    ...
    const SLM = {
        ...
        updateLGA: (stateName, lgaId) => {
            ...
            if (state) {
                ...
                state.lgas.forEach(l => {
                    const o = document.createElement('option');
                    o.value=l; o.textContent=l; lgaSel.appendChild(o);
                });
        ...
        set: (stateId, lgaId, stateVal, lgaVal) => {
            ...
            if (stateVal) {
                ...
                setTimeout(() => {
                    const ls = document.getElementById(lgaId);
                    if (ls && lgaVal) ls.value = lgaVal;
                }, 350);
            }
        }
    };
    ...
    const SM = {
        ...
        async view(id) {
            try {
                ...
                if (!s) {
                    ...
                    if (r.success) { s=r.student; AppState.cache.students.set(String(id),s); }
                }
                Swal.close();
                if (s) {
        ...
        async delete(id) {
            if (!await U.confirm('Delete Student?',"This cannot be undone!",'Yes, delete!')) return;
            try {
                await API.delete(id);
                AppState.cache.students.delete(String(id));
                await SM.fetch();
                U.ok('Student deleted.');
        ...
        async deleteMany() {
            ...
            if (!await U.confirm(`Delete ${ids.length} Students?`,'This cannot be undone!','Yes, delete!')) return;
            try {
                await API.delMany(ids);
                ids.forEach(id=>AppState.cache.students.delete(String(id)));
                await SM.fetch();
    ...
    const Pagination = {
        update: pd => {
            ...
            // Remove old page number items (keep prev/next)
            ...
            if (pd.last_page>1) {
                const cur  = pd.current_page;
                const last = pd.last_page;
                const start = Math.max(1, cur-2);
                const end   = Math.min(last, cur+2);

                if (start>1) { addPage(1); if (start>2) addDots(); }
                for (let i=start; i<=end; i++) addPage(i);
                if (end<last) { if (end<last-1) addDots(); addPage(last); }
            }
            ...
            if (prevLi && prevA) {
                if (pd.current_page<=1) {
                    ...
                    prevA.onclick = e => {
                        e.preventDefault();
                        AppState.pagination.currentPage = pd.current_page-1;
                        SM.fetch();
                    };
                }
            }
    ...
    const ViewModal = {
        ...
        populate: s => {
            ...
            if (si) {
                si.className = `position-absolute bottom-0 end-0 rounded-circle border border-2 border-white ${s.student_status==='Active'?'bg-success':'bg-secondary'}`;
                si.style.width='18px'; si.style.height='18px';
            }
        ...
        fetchTerm: async id => {
            try {
                ...
                if (r.success&&r.data) {
                    ...
                    if (thc) {
                        ...
                        thc.innerHTML=`<table class="table table-bordered table-sm">
                            <thead><tr><th>Term</th><th>Session</th><th>Class</th><th>Current</th></tr></thead>
                            <tbody><tr>
                                <td>${d.term?.term||d.term?.name||'—'}</td>
                                <td>${d.session?.session||d.session?.name||'—'}</td>
                                <td>${d.schoolClass?.schoolclass||''} ${d.schoolClass?.armRelation?.arm||''}</td>
                                <td>${d.is_current?'<span class="badge bg-success">Yes</span>':'<span class="badge bg-secondary">No</span>'}</td>
                            </tr></tbody>
                        </table>`;
                    }
                } else {
            ...
            } catch(e) { U.log('Term fetch error',e); }
        }
    };

    // ================================================================
    // EDIT FORM MANAGER — FIXED ACTION URL
    // ================================================================
    const EditForm = {
        populate: s => {
            ...
            // Always rebuild action from the stored base pattern — prevents stale URLs on repeated opens
            const form = document.getElementById('editStudentForm');
            if (form&&s.id) {
                const base = form.dataset.baseAction;
                form.action = base ? base.replace(':id',s.id) : form.action.replace(/\/[^\/]+\/([^\/]+)$/, `/${s.id}/$1`);
            }

            set('editStudentId',       s.id);
            set('editAdmissionNo',     s.admissionNo);
            set('editAdmissionYear',   s.admissionYear||new Date().getFullYear());

            // Admission date — normalise to YYYY-MM-DD
            const admDate = (s.admissionDate||s.admission_date||'').split(/[T ]/)[0];
            set('editAdmissionDate', admDate);
            ...
            // DOB & age
            const dob = (s.dateofbirth||'').split(/[T ]/)[0];
            set('editDOB', dob);
            const ageEl = document.getElementById('editAgeInput');
            if (ageEl) ageEl.value = U.age(dob)||s.age||'';
            ...
            // Admission mode — default to manual so number is visible
            const manualR = document.getElementById('editAdmissionManual');
        ...
        init: () => {
            ...
            if (si) {
                si.addEventListener('input', e => {
                    ...
                    clearTimeout(Filters._timer);
                    Filters._timer=setTimeout(()=>Filters.apply(), CONFIG.SEARCH_DEBOUNCE);
                });
                si.addEventListener('keypress', e => { if(e.key==='Enter'){clearTimeout(Filters._timer);Filters.apply();} });
            }
    ...
    const BSM = {
        show: () => {
            ...
            if (!cid||cid==='all'||!sid||sid==='all') {
                U.err('Please select both a class and a session first.','Selection Required'); return;
            }
                  ...
                  <div class="modal-body p-4">
                        ...
                        <div class="d-flex flex-wrap gap-2">
                          <div class="dropdown">
                            <button class="btn btn-outline-success btn-sm dropdown-toggle" data-bs-toggle="dropdown">
                              <i class="fas fa-user-check me-1"></i>Activity Status
                            </button>
                            <ul class="dropdown-menu shadow border-0 rounded-3 p-2">
                              <li><a class="dropdown-item rounded-2" href="#" onclick="BulkStatusManager.bulkUpdate('activity_status','Active');return false;"><i class="fas fa-check-circle text-success me-2"></i>Active</a></li>
                              <li><a class="dropdown-item rounded-2" href="#" onclick="BulkStatusManager.bulkUpdate('activity_status','Inactive');return false;"><i class="fas fa-pause-circle text-secondary me-2"></i>Inactive</a></li>
                            </ul>
                          </div>
                          <div class="dropdown">
                            <button class="btn btn-outline-warning btn-sm dropdown-toggle" data-bs-toggle="dropdown">
                              <i class="fas fa-user-tag me-1"></i>Student Type
                            </button>
                            <ul class="dropdown-menu shadow border-0 rounded-3 p-2">
                              <li><a class="dropdown-item rounded-2" href="#" onclick="BulkStatusManager.bulkUpdate('student_type','old');return false;"><i class="fas fa-history text-secondary me-2"></i>Old Student</a></li>
                              <li><a class="dropdown-item rounded-2" href="#" onclick="BulkStatusManager.bulkUpdate('student_type','new');return false;"><i class="fas fa-star text-warning me-2"></i>New Student</a></li>
                            </ul>
                          </div>
                        </div>
                      </div>
                    ...
                    <div class="table-responsive">
                      <table class="table sm-table mb-0">
                        <thead>
                          <tr>
                            <th width="46"><input class="form-check-input" type="checkbox" id="bsSelectAllTable"></th>
                            <th>Student</th><th>Admission No</th><th>Class</th>
                            <th>Activity Status</th><th>Type</th><th>Actions</th>
                          </tr>
                        </thead>
        ...
        rows: students => {
            ...
            return students.map(s=>{
                const init=U.initials(s.firstname,s.lastname);
                const act=s.student_status==='Active'
                    ? '<span class="sm-badge badge-active" style="font-size:11px;">Active</span>'
                    : '<span class="sm-badge badge-inactive" style="font-size:11px;">Inactive</span>';
                const type=s.statusId==2
                    ? '<span class="sm-badge badge-new" style="font-size:11px;">New</span>'
                    : '<span class="sm-badge badge-old" style="font-size:11px;">Old</span>';
                return `<tr data-student-id="${s.id}">
                  <td><input class="form-check-input bs-checkbox" type="checkbox" value="${s.id}"></td>
                  <td>
                    <div class="d-flex align-items-center gap-2">
                      <div class="sm-avatar-init" style="width:36px;height:36px;border-radius:8px;font-size:13px;">${U.esc(init)}</div>
                      <div>
                        <div class="stu-name">${U.esc(s.lastname||'')} ${U.esc(s.firstname||'')}</div>
                        <small class="text-muted">${U.esc(s.othername||'')}</small>
                      </div>
                    </div>
                  </td>
                  ...
                  <td>
                    <div class="d-flex align-items-center gap-1">
                      ${act}
                      <button class="act-btn act-edit" style="width:26px;height:26px;" onclick="BulkStatusManager.toggleOne(${s.id},'activity')" title="Toggle"><i class="fas fa-exchange-alt" style="font-size:11px;"></i></button>
                    </div>
                  </td>
                  <td>
                    <div class="d-flex align-items-center gap-1">
                      ${type}
                      <button class="act-btn act-edit" style="width:26px;height:26px;" onclick="BulkStatusManager.toggleOne(${s.id},'type')" title="Toggle"><i class="fas fa-exchange-alt" style="font-size:11px;"></i></button>
                    </div>
                  </td>
        ...
        bulkUpdate: async (type,value) => {
            const ids=BSM.getSelectedIds();
            if (!ids.length) { U.err('Please select at least one student.','No Selection'); return; }
            const label=type==='student_type'?(value==='old'?'Old Student':'New Student'):value;
            if (!await U.confirm(`Update ${ids.length} student(s)?`,`Set to "${label}"`,'Yes, update')) return;
            Swal.fire({title:'Updating…',allowOutsideClick:false,didOpen:()=>Swal.showLoading()});
            try {
                const r=await API.bulkStatus({student_ids:ids,update_type:type,value});
                Swal.close();
                if (r.success) { U.ok(r.message); BSM.refreshData(); }
            } catch(e) { Swal.close(); U.err(e.response?.data?.message||e.message); }
        },
        ...
        toggleOne: async (id,type) => {
            ...
            try {
                ...
                Swal.close();
                if (r.success) { U.ok('Updated'); BSM.refreshData(); }
    ...
    const TRM = {
        show: () => {
            ...
            const sid=document.getElementById('session-filter')?.value;
            if (!tid||tid==='all'||!sid||sid==='all') {
                U.err('Please select both a term and a session first.','Selection Required'); return;
            }
            AppState.termFilters={
                term_id:tid, session_id:sid,
                class_id: document.getElementById('schoolclass-filter')?.value!=='all'
                    ? document.getElementById('schoolclass-filter')?.value : null
            };
            Swal.fire({title:'Loading…',allowOutsideClick:false,didOpen:()=>Swal.showLoading()});
                ...
                <div class="modal-content">
                  ...
                  <div class="modal-body p-4">
                    ...
                    <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
                      <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="trmSelectAll">
                        <label class="form-check-label fw-semibold" for="trmSelectAll">Select All</label>
                      </div>
                      <button class="btn btn-danger btn-sm rounded-3" onclick="TermRegistrationManager.bulkRemove()">
                ...
                return `<div class="col-md-4 col-lg-3">
                  <div class="stu-card" data-registration-id="${s.registration_id}" style="margin-bottom:12px;">
                    <div class="stu-card-header" style="min-height:80px;">
                      ${cur}
                      <div class="stu-card-checkbox">
                        <input class="form-check-input trm-checkbox" type="checkbox" value="${s.registration_id}" style="border:2px solid rgba(255,255,255,.8);width:18px;height:18px;">
                      </div>
        ...
        refreshData: async () => {
            ...
            try {
                ...
                if (r.success) {
                    const c=document.getElementById('termStudentsContainer');
                    if (c) c.innerHTML=TRM.cards(r.students);
                }
                Swal.close();
            } catch(e) { Swal.close(); U.err('Failed.'); }
        }
        ...
        generate: async () => {
            ...
            try {
                ...
                if (e.response?.data instanceof Blob) {
                    try { const t=await e.response.data.text(); msg=JSON.parse(t).message||msg; } catch{}
    ...
    document.getElementById('addStudentForm')?.addEventListener('submit', async e=>{
        ...
        try {
            ...
            if (r.data.success) {
                ...
                await SM.fetch();
                U.ok(r.data.message||'Student registered.');
                form.reset();
                const av=document.getElementById('addStudentAvatar');
                if (av) av.src='[via.placeholder.com](https://via.placeholder.com/120x120/2563eb/ffffff?text=Photo)';
            } else {
                U.err(r.data.message||'Failed to save student.');
            }
        } catch(err) {
            Swal.close();
            U.err(err.response?.data?.message||'Failed to save student.');
        }
    });
    ...
    document.getElementById('editStudentForm')?.addEventListener('submit', async e=>{
        ...
        try {
            ...
            if (r.data.success) {
                ...
                // Invalidate cache for this student so fresh data is loaded next time
                const sid=document.getElementById('editStudentId')?.value;
                if (sid) AppState.cache.students.delete(String(sid));
                await SM.fetch();
                U.ok(r.data.message||'Student updated.');
            } else {
                U.err(r.data.message||'Failed to update student.');
            }
        } catch(err) {
            Swal.close();
            U.err(err.response?.data?.message||'Failed to update student.');
        }
    });
    ...
    document.addEventListener('click', e=>{
        ...
        if (vb) { e.preventDefault(); SM.view(vb.dataset.studentId); return; }
        const eb=e.target.closest('.edit-student-btn');
        if (eb) { e.preventDefault(); SM.edit(eb.dataset.studentId); return; }
        const db=e.target.closest('.delete-student-btn');
        if (db) { e.preventDefault(); SM.delete(db.dataset.studentId); return; }
    });
    ...
    window.updateAdmissionNumber = (prefix='')=>{
        const yrId  = prefix ? `${prefix}AdmissionYear` : 'admissionYear';
    ...
    window.toggleAdmissionInput = (prefix='')=>{
        const autoId   = prefix ? `${prefix}AdmissionAuto`   : 'admissionAuto';
        const noId     = prefix ? `${prefix}AdmissionNo`     : 'admissionNo';
        const selMode  = document.getElementById(autoId)?.checked ? 'auto' : 'manual';
        const input    = document.getElementById(noId);
        if (input) input.readOnly = selMode==='auto';
    };
    ...
    // Expose managers needed by dynamically-created modal onclicks
