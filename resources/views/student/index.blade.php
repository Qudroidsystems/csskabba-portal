    ...
    <div class="page-content">
        <div class="container-fluid">
            ...
            <style>
                ...
                /* ====================================================
                   ACTION BUTTONS — PROFESSIONAL TABLE CRUD
                   ==================================================== */
                .act-btn {
                ...
                .act-view:hover  { background: #0284c7; color: #fff !important; transform: translateY(-2px); box-shadow: 0 4px 10px rgba(2,132,199,.3); }
                .act-edit   { background: #fef9c3; color: #a16207; }
                .act-edit:hover  { background: #d97706; color: #fff !important; transform: translateY(-2px); box-shadow: 0 4px 10px rgba(217,119,6,.3); }
                .act-delete { background: #fee2e2; color: #dc2626; }
                .act-delete:hover { background: #dc2626; color: #fff !important; transform: translateY(-2px); box-shadow: 0 4px 10px rgba(220,38,38,.3); }
                ...
                .stu-card-edit   { background: #f3f4f6; color: #374151; border: 1px solid var(--sm-border) !important; }
                .stu-card-edit:hover   { background: #e5e7eb; }
                .stu-card-delete { background: #fef2f2; color: var(--sm-danger); border: 1px solid #fecaca !important; }
                ...
                .sm-pagination .page-item.disabled .page-link { opacity: .4; cursor: not-allowed; }
                ...
                .order-badge { font-size: 10px; padding: 1px 5px; border-radius: 8px; background: var(--sm-accent); color: #fff; margin-left: 4px; }
                ...
                .filter-btn-primary { background: var(--sm-accent); color: #fff; border-color: var(--sm-accent); }
                .filter-btn-primary:hover { background: #1d4ed8; color: #fff; }
                .filter-btn-secondary { background: #fff; color: #374151; border-color: var(--sm-border); }
                .filter-btn-secondary:hover { background: #f3f4f6; }
                .filter-btn-warning  { background: #f59e0b; color: #fff; border-color: #f59e0b; }
                .filter-btn-warning:hover  { background: #d97706; color: #fff; }
                .filter-btn-info     { background: #0ea5e9; color: #fff; border-color: #0ea5e9; }
                .filter-btn-info:hover     { background: #0284c7; color: #fff; }
            ...
            <div class="sm-panel">
                    ...
                    <div class="d-flex flex-wrap align-items-center gap-2">
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu shadow-sm border-0 rounded-3 p-2">
                                <li><a class="dropdown-item rounded-2 py-2" href="javascript:void(0);" id="deleteMultipleBtn">
                                    <i class="fas fa-trash text-danger me-2"></i>Delete Selected
                                </a></li>
                                <li><a class="dropdown-item rounded-2 py-2" href="javascript:void(0);" id="updateCurrentTermBtn">
                ...
                <div class="sm-filter">
                    <div class="row g-2 align-items-end">
                        ...
                        <div class="col-md-1 col-6">
                            <button class="filter-btn filter-btn-primary w-100" id="filterBtn" title="Apply Filters">
                                <i class="fas fa-filter"></i><span class="d-none d-lg-inline">Filter</span>
                            </button>
                        </div>
                        <div class="col-md-1 col-6">
                            <button class="filter-btn filter-btn-secondary w-100" id="resetFiltersBtn" title="Reset Filters">
                                <i class="fas fa-redo-alt"></i><span class="d-none d-lg-inline">Reset</span>
                            </button>
                        </div>
                        <div class="col-md-1 col-6">
                            <button class="filter-btn filter-btn-warning w-100" id="bulkStatusBtn" title="Bulk Update Student Status">
                                <i class="fas fa-sync-alt"></i><span class="d-none d-lg-inline">Status</span>
                            </button>
                        </div>
                        <div class="col-md-1 col-6">
                            <button class="filter-btn filter-btn-info w-100" id="manageTermBtn" title="Manage Term Registrations">
                                <i class="fas fa-calendar-alt"></i><span class="d-none d-lg-inline">Term</span>
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div id="emptyState" class="sm-empty d-none">
                    ...
                    <button class="btn-pg" id="resetFromEmptyBtn"><i class="fas fa-redo"></i>Reset Filters</button>
                </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn-pg" id="confirmUpdateCurrentTerm">
                            <i class="fas fa-save"></i>Register / Update
                        </button>
                    </div>
                </div>
            </div>
        ...
        <div id="printStudentReportModal" class="modal fade" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
                <div class="modal-content">
                    <div class="modal-header modal-hdr">
                        <h5 class="modal-title"><i class="fas fa-file-export me-2"></i>Generate Student Report</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body p-4">
                        <form id="printReportForm">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="alert alert-info py-2 px-3 mb-2" style="font-size:12px;">
                                    ...
                                    Drag the <i class="fas fa-grip-vertical"></i> handle to reorder columns. Check to include in report.
                                </div>
                                ...
                                <div class="mt-2 d-flex gap-2">
                                    <button type="button" class="btn btn-sm btn-outline-primary rounded-3" id="selectAllColumnsBtn">
                                        <i class="fas fa-check-square me-1"></i>Check All
                                    </button>
                                    <button type="button" class="btn btn-sm btn-outline-secondary rounded-3" id="deselectAllColumnsBtn">
                ...
                <div class="modal-content">
                    ...
                    <form id="addStudentForm" enctype="multipart/form-data" autocomplete="off" method="POST" action="{{ route('student.store') }}">
                        ...
                        <div class="modal-footer bg-light">
                            ...
                            <button type="submit" class="btn-pg" id="add-btn"><i class="fas fa-save"></i>Register Student</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="modal-dialog modal-dialog-centered modal-xl modal-dialog-scrollable">
                <div class="modal-content">
                    ...
                    <form id="editStudentForm" enctype="multipart/form-data" autocomplete="off" method="POST"
                        ...
                        <div class="modal-footer bg-light">
                            <button type="button" class="btn btn-secondary rounded-3" data-bs-dismiss="modal"><i class="fas fa-times me-1"></i>Cancel</button>
                            <button type="submit" class="btn-pg" id="edit-btn"><i class="fas fa-save"></i>Update Student</button>
                        </div>
                    </form>
                </div>
            </div>
...
(function () {
    ...
    const U = {
        ...
        hideLoading: () => {
            ...
            const hasData = AppState.pagination.data && AppState.pagination.data.length > 0;
            if (hasData) {
        ...
        confirm: async (title, text, confirmText='Yes') => {
            const r = await Swal.fire({title, text, icon:'warning', showCancelButton:true, confirmButtonText:confirmText, cancelButtonText:'Cancel', customClass:{confirmButton:'btn btn-danger me-2', cancelButton:'btn btn-light'}});
    ...
    const SLM = {
        populate: (selectId, lgaId) => {
            ...
            sel.addEventListener('change', () => SLM.updateLGA(sel.value, lgaId));
        },
        updateLGA: (stateName, lgaId) => {
            ...
            if (state) {
                ...
                state.lgas.forEach(l => { const o = document.createElement('option'); o.value = l; o.textContent = l; lgaSel.appendChild(o); });
            } else {
    ...
    const Render = {
        table: students => {
            ...
            students.forEach(s => {
                ...
                const typeBadge = parseInt(s.statusId) === 2
                ...
                tr.innerHTML = `
                    ...
                    <td>
                        <div class="d-flex gap-1 justify-content-center align-items-center">
                            <button class="act-btn act-view view-student-btn" data-student-id="${s.id}" title="View Student"><i class="fas fa-eye"></i></button>
                            <button class="act-btn act-edit edit-student-btn" data-student-id="${s.id}" title="Edit Student"><i class="fas fa-edit"></i></button>
                            <button class="act-btn act-delete delete-student-btn" data-student-id="${s.id}" title="Delete Student"><i class="fas fa-trash-alt"></i></button>
                        </div>
                    </td>`;
                frag.appendChild(tr);
            });
        ...
        cards: students => {
            ...
            if (!container) return;
            if (!students || !students.length) { container.innerHTML = ''; return; }
            const frag = document.createDocumentFragment();
            students.forEach(s => {
                ...
                const typeBadge = parseInt(s.statusId) === 2
                    ? `<span class="sm-badge badge-new ms-1" style="font-size:11px;"><i class="fas fa-star" style="font-size:9px"></i>New</span>`
                    : `<span class="sm-badge badge-old ms-1" style="font-size:11px;"><i class="fas fa-history" style="font-size:9px"></i>Old</span>`;
                    ...
                    <div class="stu-card mb-3" data-id="${s.id}">
                        ...
                        <div class="stu-card-body">
                            ...
                            <div class="stu-card-actions">
                                <button class="stu-card-view view-student-btn" data-student-id="${s.id}"><i class="fas fa-eye me-1"></i>View</button>
                                <button class="stu-card-edit edit-student-btn" data-student-id="${s.id}"><i class="fas fa-edit me-1"></i>Edit</button>
        ...
        updateCheckAll: () => {
            ...
            ['checkAll','checkAllTable'].forEach(id => {
                ...
                if (el) { el.checked = total > 0 && total === checked; el.indeterminate = checked > 0 && checked < total; }
            });
            const bulkBtn = document.getElementById('bulkActionsDropdown');
            if (bulkBtn) {
                bulkBtn.disabled = checked === 0;
                bulkBtn.innerHTML = checked > 0
                    ? `<i class="fas fa-cog me-1"></i>Bulk Actions (${checked})`
                    : '<i class="fas fa-cog me-1"></i>Bulk Actions';
            }
        },
        ...
        toggleView: type => {
            ...
            if (type === 'table') {
                ...
                if (AppState.pagination.data && AppState.pagination.data.length) Render.table(AppState.pagination.data);
            } else {
                tv?.classList.add('d-none'); cv?.classList.remove('d-none');
                tvb?.classList.remove('active'); cvb?.classList.add('active');
                if (AppState.pagination.data && AppState.pagination.data.length) Render.cards(AppState.pagination.data);
            }
        }
    };
    ...
    const Pagination = {
        update: pd => {
            ...
            // Remove existing page numbers (keep prev/next)
            ...
            if (pd.last_page > 1) {
                ...
                const addItem = (n) => {
                    ...
                    a.textContent = n;
                    a.addEventListener('click', () => { AppState.pagination.currentPage = n; SM.fetch(); });
                    li.appendChild(a);
                ...
                if (start > 1) { addItem(1); if (start > 2) addDots(); }
                for (let i = start; i <= end; i++) addItem(i);
                if (end < pd.last_page) { if (end < pd.last_page - 1) addDots(); addItem(pd.last_page); }
            }
            ...
            if (prevLi && prevA) {
                if (pd.current_page <= 1) {
                    ...
                    prevA.onclick = e => { e.preventDefault(); AppState.pagination.currentPage = pd.current_page - 1; SM.fetch(); };
                }
            }
            ...
            if (nextLi && nextA) {
                if (pd.current_page >= pd.last_page) {
                    ...
                    nextA.onclick = e => { e.preventDefault(); AppState.pagination.currentPage = pd.current_page + 1; SM.fetch(); };
                }
            }
        }
    };
    ...
    const SM = {
        ...
        async edit(id) {
            try {
                Swal.fire({title:'Loading…', allowOutsideClick:false, didOpen:()=>Swal.showLoading()});
                // Always re-init state dropdowns so LGA re-populates correctly
        ...
        async delete(id) {
            if (!await U.confirm('Delete Student?', "This action cannot be undone!", 'Yes, delete it!')) return;
            try {
                await API.delete(id);
        ...
        async deleteMany() {
            ...
            try {
                ...
                await SM.fetch();
                U.ok(`${ids.length} student(s) deleted successfully.`);
                Selection.clearAll();
    ...
    const ViewModal = {
        ...
        populate: s => {
            ...
            const age = s.age || U.age(s.dateofbirth) || '—';
            U.setText('viewAge',             age);
            U.setText('viewAgeDetail',       age);
            ...
            const type = parseInt(s.statusId) === 2 ? 'New Student' : parseInt(s.statusId) === 1 ? 'Old Student' : '—';
            ...
            if (tb) {
                if (parseInt(s.statusId) === 2) { tb.style.background='#fef3c7'; tb.style.color='#92400e'; }
        ...
        fetchTerm: async id => {
            try {
                ...
                if (r.success && r.data) {
                    ...
                    U.setText('viewCurrentTerm',    '—');
                    U.setText('viewCurrentSession',  '—');
                    const status = document.getElementById('viewCurrentTermStatus');
                ...
                if (thc) thc.style.display = 'block';
            }
        }
    };
    ...
    const EditForm = {
        populate: s => {
            ...
            const chk = (name, v, scope='') => {
                const el = document.querySelector(`#editStudentForm input[name="${name}"][value="${v}"]`);
            ...
            // Radio buttons
            chk('statusId',      String(s.statusId));
            chk('student_status', s.student_status);
            chk('gender',         s.gender);

            // Admission mode — default to manual so existing no is preserved
            ...
            if (houseEl && (s.schoolhouseid || s.schoolhouse)) {
                houseEl.value = s.schoolhouseid || s.schoolhouse;
            }

            // State & LGA (re-populate cleanly)
            if (s.state) {
                SLM.set('editState', 'editLocal', s.state, s.local);
            }
        ...
        init: () => {
            ...
            if (si) {
                ...
                si.addEventListener('keypress', e => {
                    if (e.key === 'Enter') { clearTimeout(Filters.searchDebounce); Filters.apply(); }
                });
            }
            if (cl) cl.addEventListener('click', () => {
                if (si) { si.value = ''; cl.style.display = 'none'; }
                Filters.apply();
            });
    ...
    const BSM = {
        show: () => {
            ...
            if (!cid || cid === 'all' || !sid || sid === 'all') {
                U.err('Please select both a class and a session first.', 'Selection Required');
                return;
            }
                ...
                <div class="modal-content">
                  ...
                  <div class="modal-body p-4">
                    <div class="row g-3 mb-4">
                      ...
                      <div class="col-6 col-md-3"><div class="sm-stat" style="--sm-stat-color:#6b7280"><div class="sm-stat-value">${stats.inactive}</div><div class="sm-stat-label">Inactive</div></div></div>
                        ...
                        <div class="d-flex flex-wrap gap-2">
                          <div class="dropdown">
                            ...
                            <ul class="dropdown-menu shadow border-0 rounded-3 p-2">
                              <li><a class="dropdown-item rounded-2 py-2" href="javascript:void(0)" onclick="BulkStatusManager.bulkUpdateStatus('activity_status','Active')"><i class="fas fa-check-circle text-success me-2"></i>Set Active</a></li>
                              <li><a class="dropdown-item rounded-2 py-2" href="javascript:void(0)" onclick="BulkStatusManager.bulkUpdateStatus('activity_status','Inactive')"><i class="fas fa-pause-circle text-secondary me-2"></i>Set Inactive</a></li>
                            </ul>
                          </div>
                          <div class="dropdown">
                            ...
                            <ul class="dropdown-menu shadow border-0 rounded-3 p-2">
                              <li><a class="dropdown-item rounded-2 py-2" href="javascript:void(0)" onclick="BulkStatusManager.bulkUpdateStatus('student_type','old')"><i class="fas fa-history text-secondary me-2"></i>Set Old Student</a></li>
                              <li><a class="dropdown-item rounded-2 py-2" href="javascript:void(0)" onclick="BulkStatusManager.bulkUpdateStatus('student_type','new')"><i class="fas fa-star text-warning me-2"></i>Set New Student</a></li>
                            </ul>
                          </div>
                        </div>
                      </div>
                  ...
                  <div class="modal-footer">
                    <button type="button" class="btn btn-light rounded-3 fw-semibold" data-bs-dismiss="modal"><i class="fas fa-times me-1"></i>Close</button>
                    <button type="button" class="btn-pg" onclick="BulkStatusManager.refreshData()"><i class="fas fa-sync-alt"></i>Refresh</button>
                  </div>
                </div>
              </div>
            </div>`;
            document.body.insertAdjacentHTML('beforeend', html);
            const el = document.getElementById('bsSelectAll');
            const el2 = document.getElementById('bsSelectAllTable');
            const handler = e => { document.querySelectorAll('.bs-checkbox').forEach(cb => cb.checked = e.target.checked); };
        ...
        rows: students => {
            ...
            return students.map(s => {
                ...
                const act = s.student_status === 'Active'
                    ? '<span class="sm-badge badge-active" style="font-size:11px;"><i class="fas fa-circle" style="font-size:7px"></i>Active</span>'
                    : '<span class="sm-badge badge-inactive" style="font-size:11px;"><i class="fas fa-circle" style="font-size:7px"></i>Inactive</span>';
                const type = parseInt(s.statusId) === 2
                    ? '<span class="sm-badge badge-new" style="font-size:11px;"><i class="fas fa-star" style="font-size:9px"></i>New</span>'
                    : '<span class="sm-badge badge-old" style="font-size:11px;"><i class="fas fa-history" style="font-size:9px"></i>Old</span>';
                return `<tr data-student-id="${s.id}">
                  ...
                  <td>
                    <div class="d-flex align-items-center gap-1">
                      ${act}
                      <button class="act-btn act-edit" style="width:28px;height:28px;" onclick="BulkStatusManager.toggleOne(${s.id},'activity')" title="Toggle Status"><i class="fas fa-exchange-alt" style="font-size:11px;"></i></button>
                    </div>
                  </td>
                  <td>
                    <div class="d-flex align-items-center gap-1">
                      ${type}
                      <button class="act-btn act-edit" style="width:28px;height:28px;" onclick="BulkStatusManager.toggleOne(${s.id},'type')" title="Toggle Type"><i class="fas fa-exchange-alt" style="font-size:11px;"></i></button>
                    </div>
                  </td>
                  <td>
                    <button class="act-btn act-view" onclick="StudentManager.viewStudent(${s.id})" title="View"><i class="fas fa-eye"></i></button>
                  </td>
                </tr>`;
            }).join('');
        },
        ...
        toggleOne: async (id, type) => {
            ...
            const newVal = type === 'activity' ? (s.student_status === 'Active' ? 'Inactive' : 'Active') : (parseInt(s.statusId) === 1 ? 'new' : 'old');
            ...
            try {
                ...
                Swal.close();
                if (r.success) { U.ok('Updated successfully.'); BSM.refreshData(); }
    ...
    const TRM = {
        show: () => {
            ...
            if (!tid || tid === 'all' || !sid || sid === 'all') {
                U.err('Please select both a term and a session first.', 'Selection Required');
                return;
            }
            ...
            <div class="modal fade" id="termStudentsModal" tabindex="-1" data-bs-backdrop="static">
              <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
                <div class="modal-content">
                  ...
                  <div class="modal-body p-4">
                    ...
                    <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
                      <div class="form-check">
                        ...
                        <label class="form-check-label fw-semibold" for="trmSelectAll">Select All Students</label>
                      </div>
                      <button class="btn btn-danger fw-semibold rounded-3" onclick="TermRegistrationManager.bulkRemove()">
                  ...
                  <div class="modal-footer">
                    <button type="button" class="btn btn-light rounded-3 fw-semibold" data-bs-dismiss="modal"><i class="fas fa-times me-1"></i>Close</button>
                    <button type="button" class="btn-pg" onclick="TermRegistrationManager.refreshData()"><i class="fas fa-sync-alt"></i>Refresh</button>
                  </div>
                </div>
              </div>
            </div>`;
        ...
        cards: students => {
            ...
            return students.map(s => {
                ...
                const curBadge = s.is_current ? '<span class="badge bg-success position-absolute top-0 end-0 m-2" style="font-size:10px;">Current</span>' : '';
                return `<div class="col-md-4 col-lg-3">
                  <div class="stu-card mb-3" data-registration-id="${s.registration_id}">
                    <div class="stu-card-header" style="min-height:80px;position:relative;">
                      ${curBadge}
                      <div class="stu-card-checkbox">
                    ...
                    <div class="stu-card-body">
                      ...
                      <button class="btn btn-outline-danger btn-sm w-100 rounded-3 fw-semibold"
        ...
        removeOne: async (regId, name) => {
            ...
            try {
                const r = await API.rmTerm(regId);
                Swal.close();
                if (r.success) {
                    U.ok(r.message || 'Student removed.');
        ...
        bulkRemove: async () => {
            ...
            Swal.fire({title:'Removing…', allowOutsideClick:false, didOpen:()=>Swal.showLoading()});
            try {
                const r = await API.rmTermMany(ids);
                Swal.close();
                if (r.success) { U.ok(r.message || 'Students removed.'); TRM.refreshData(); }
            } catch(e) { Swal.close(); U.err('Failed to remove.'); }
        },
    ...
    const Report = {
        ...
        init: () => {
            ...
            container.querySelectorAll('.column-checkbox').forEach(cb => {
                cb.addEventListener('change', () => { Report.updateOrder(); Report.updatePreview(); });
            });
        ...
        updatePreview: () => {
            ...
            const labels = Array.from(document.querySelectorAll('.column-checkbox:checked')).map(cb => {
                return document.querySelector(`label[for="${cb.id}"]`)?.textContent.trim() || cb.value;
            });
    ...
    window.updateAdmissionNumber = (prefix = '') => {
        const yr    = document.getElementById(prefix ? `${prefix}AdmissionYear` : 'admissionYear')?.value || new Date().getFullYear();
        const input = document.getElementById(prefix ? `${prefix}AdmissionNo`   : 'admissionNo');
        if (input) { const base = `TCC/${yr}/`; if (!input.value.startsWith(base)) input.value = `${base}0001`; }
    };
    window.toggleAdmissionInput = (prefix = '') => {
        const sel   = document.querySelector(prefix ? `input[name="admissionMode"][id^="${prefix}"]:checked` : 'input[name="admissionMode"]:checked');
        const input = document.getElementById(prefix ? `${prefix}AdmissionNo` : 'admissionNo');
        if (input && sel) input.readOnly = sel.value === 'auto';
    };
    ...
    // Expose managers for inline onclick in dynamically created modals
