...
<!-- Add Student Modal (simplified - keep your existing working modal) -->
        ...
        <div class="modal-content">
            ...
            <form id="addStudentForm" enctype="multipart/form-data" method="POST" action="{{ route('student.store') }}">
                ...
                <div class="modal-body p-4">
                    <!-- Your existing add student form content here -->
                    <div class="alert alert-info">Add Student Form Content - Keep your original working form</div>
                </div>
...
<div id="editStudentModal" class="modal fade" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            ...
            <form id="editStudentForm" enctype="multipart/form-data" method="POST" action="">
                @csrf
                @method('PATCH')
                <div class="modal-body p-4">
                    <input type="hidden" id="editStudentId" name="id">
                    <div class="alert alert-info">Edit Student Form Content - Keep your original working form</div>
                </div>
                <div class="modal-footer bg-light">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
...
<div id="viewStudentModal" class="modal fade" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-body p-4">
                <div class="alert alert-info">View Student Content</div>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body p-4">
                <form id="updateCurrentTermForm">
                    @csrf
                    <div class="alert alert-info">Registering term for <span id="selectedStudentsCount">0</span> student(s)</div>
                    <div class="mb-3">
...
(function() {
    ...
    const API = {
        ...
        async deleteStudent(id) {
            return await axios.delete(`/student/${id}/destroy`);
        },

        async deleteMultiple(ids) {
            return await axios.post('/students/destroy-multiple', { ids });
        },

        async updateBulkTerm(data) {
            return await axios.post('/student-current-term/bulk-update', data);
        },
                ...
                return `
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center gap-2">
                                <div class="avatar-sm">
                                    <span class="avatar-title rounded-circle bg-primary text-white">${(s.firstname?.charAt(0) || '')}${(s.lastname?.charAt(0) || '')}</span>
                                </div>
                                <div>
                                    <h6 class="mb-0">${Utils.escapeHtml(s.lastname || '')} ${Utils.escapeHtml(s.firstname || '')}</h6>
                                    <small class="text-muted">${Utils.escapeHtml(s.admissionNo || 'N/A')}</small>
                                </div>
                            </div>
                        </td>
                    ...
                    <div class="col-xl-3 col-lg-4 col-md-6 mb-4">
                        <div class="student-profile-card">
                            <div class="checkbox-container">
                                <div class="form-check"><input class="form-check-input student-checkbox" type="checkbox" value="${s.id}"></div>
                            </div>
                            <div class="card-header">
                                <div class="header-content">
                                    <h5 class="student-name">${Utils.escapeHtml(s.lastname || '')} ${Utils.escapeHtml(s.firstname || '')}</h5>
                                    <span class="student-admission">${Utils.escapeHtml(s.admissionNo || 'N/A')}</span>
                                </div>
                                <div class="avatar-container">
                                    <div class="avatar-initials">${(s.firstname?.charAt(0) || '')}${(s.lastname?.charAt(0) || '')}</div>
                                </div>
                            </div>
                            <div class="card-body">
                                <div>
                                    <span class="status-badge ${statusClass}"><i class="fas fa-circle me-1"></i>${s.student_status || 'Inactive'}</span>
                                    ${s.statusId == 2 ? '<span class="status-badge status-new ms-2"><i class="fas fa-star me-1"></i>New</span>' : (s.statusId == 1 ? '<span class="status-badge status-old ms-2"><i class="fas fa-history me-1"></i>Old</span>' : '')}
                                </div>
        ...
        updateCheckAll: () => {
            ...
            if (bulkBtn) {
                ...
                bulkBtn.innerHTML = checked > 0
                    ? `<i class="fas fa-cog me-2"></i>Actions (${checked})`
                    : `<i class="fas fa-cog me-2"></i>Actions`;
            }
        },
    ...
    const StudentManager = {
            ...
            try {
                ...
                if (response.success && response.data) {
                    ...
                    if (currentView === 'table') {
                        Render.table(pagination.data);
                    } else {
                        Render.cards(pagination.data);
                    }
        ...
        async deleteMultiple() {
            ...
            if (ids.length === 0) {
                Utils.showError('Please select at least one student');
                return;
            }
            if (!await Utils.showConfirm(`Delete ${ids.length} Students?`, 'This action cannot be undone!')) return;
            try {
                await API.deleteMultiple(ids);
                Utils.showSuccess(`${ids.length} student(s) deleted`);
                await this.fetchStudents();
            } catch (error) {
                Utils.showError('Failed to delete students');
            }
        }
    };
    ...
    const CurrentTermManager = {
        ...
        async updateTerm() {
            ...
            if (!classId || !termId || !sessionId) {
                Utils.showError('Please select class, term, and session');
                return;
            }
            ...
            try {
                ...
                Utils.showError('Failed to update term');
            }
        }
    };
    ...
    function setupEventListeners() {
        ...
        // Student action buttons (delegation)
        document.addEventListener('click', (e) => {
            const viewBtn = e.target.closest('.view-student-btn');
            if (viewBtn) {
                console.log('View student:', viewBtn.dataset.id);
            ...
            if (editBtn) {
                console.log('Edit student:', editBtn.dataset.id);
            ...
            if (deleteBtn) {
                StudentManager.deleteStudent(deleteBtn.dataset.id);
                return;
            }
        });
        ...
        document.getElementById('pagination')?.addEventListener('click', (e) => {
            ...
            if (link && link.dataset.page) {
                e.preventDefault();
                PaginationHandler.goToPage(parseInt(link.dataset.page));
            }
        });
