    ...
    <style>
        *{
        ...
        .student-section{
            ...
            /* Full A4 Height - No bottom space */
            min-height:289mm;
            display:flex;
...
<body>
    ...
    @php
        ...
        $defaultColumns  = ['sn', 'admission_no', 'name', 'total', 'bf', 'cum', 'grade', 'position', 'position_total', 'arm_position', 'arm_position_cum', 'class_average'];
    ...
    @foreach ($allStudentData as $index => $studentData)
        @php
            ...
            $phones = is_array($schoolInfo->school_phones ?? null) ? $schoolInfo->school_phones : (json_decode($schoolInfo->school_phones ?? '[]', true) ?? []);
                            ...
                            <div class="school-logo">
                                @php
                                    $logoSrc = !empty($studentData['school_logo_base64'])
                                        ...
                                        : 'data:image/svg+xml;base64,' . base64_encode('<svg xmlns="http://www.w3.org/2000/svg" width="68" height="80" viewBox="0 0 100 100"><rect width="100" height="100" fill="#f8f9fa" stroke="#47b492" stroke-width="2"/><circle cx="50" cy="40" r="15" fill="#47b492" opacity="0.6"/><rect x="35" y="60" width="30" height="20" fill="#47b492" opacity="0.6" rx="3"/></svg>');
                                @endphp
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                @if(in_array('position', $columnsToShow))
                                    <th class="col-position">Class Pos<br><span style="font-size:4.5px;">(Cum)</span></th>
                                @endif
                                @if(in_array('position_total', $columnsToShow))
                                    <th class="col-position">Class Pos<br><span style="font-size:4.5px;">(Total)</span></th>
                                @endif
                                @if(in_array('arm_position', $columnsToShow))
                                    <th class="col-position">Arm Pos<br><span style="font-size:4.5px;">(Total)</span></th>
                                @endif
                                @if(in_array('arm_position_cum', $columnsToShow))
                                    <th class="col-position">Arm Pos<br><span style="font-size:4.5px;">(Cum)</span></th>
                                @endif
                        ...
                        <tbody>
                            ...
                            @php
                                ...
                                $rawPos = $score->position ?? null;
                                $rawPosTotal = $score->position_total ?? null;
                                $rawArmPos = $score->arm_position ?? null;
                                $rawArmPosCum = $score->arm_position_cum ?? null;

                                $bf = (float)($score->bf ?? 0);
                                if ($bf == 0 && $rawPos === null && $rawPosTotal !== null) $rawPos = $rawPosTotal;
                                if ($bf == 0 && $rawArmPosCum === null && $rawArmPos !== null) $rawArmPosCum = $rawArmPos;
                            ...
                            <tr>
                                ...
                                @endif
                                @if(in_array('bf', $columnsToShow)) <td>{{ isset($score->bf) ? number_format($score->bf, 1) : '-' }}</td> @endif
                            ...
                            @empty
                            <tr><td colspan="{{ $totalColCount }}" style="text-align:center;padding:8px;">No scores available.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                    ...
                    <tr>
                        <td style="width:70px; text-align:center; vertical-align:middle; padding:4px;">
                            @if(!empty($studentData['school_stamp_base64']))
                                <img src="{{ $studentData['school_stamp_base64'] }}" alt="Stamp" style="width:60px;height:60px;opacity:0.35;display:block;margin:0 auto;">
                            @else
                                <svg class="stamp-inline" viewBox="0 0 100 100" fill="none" style="width:60px;height:60px;opacity:0.35;">
