{{--
    ...
    Data shape: flat object from raw LEFT JOIN query in StudentIdCardController::fetchStudentsForCards()
    Available columns: id, admissionNo, firstname, lastname, othername, gender, dateofbirth,
                       picture (filename), schoolclass, arm, session, term
--}}
@php
    ...
    $qrData = ($student->admissionNo ?? '') . '|' . $fullname . '|' . ($classArm ?: 'N/A');
    $qrB64  = base64_encode(
...
">
    ...
    <div style="background:linear-gradient(150deg,#1e3a5f 0%,#2169ad 60%,#1e3a5f 100%);
        padding:18px 14px 14px;text-align:center;position:relative;overflow:hidden;">
        ...
        @if($logoUrl)
            <img src="{{ $logoUrl }}" style="height:44px;width:44px;object-fit:contain;
                border-radius:50%;border:2px solid rgba(255,255,255,.4);
                display:block;margin:0 auto 6px;" alt="logo">
        @else
            <div style="width:44px;height:44px;border-radius:50%;border:2px solid rgba(255,255,255,.4);
                background:rgba(255,255,255,.15);display:flex;align-items:center;
                justify-content:center;margin:0 auto 6px;font-size:20px;color:#fff;">&#127979;</div>
        @endif

        <div style="color:#fff;font-weight:800;font-size:12.5px;letter-spacing:.3px;
        ...
        <div style="display:inline-block;background:rgba(255,255,255,.18);
            border:1px solid rgba(255,255,255,.3);color:#fff;font-size:8px;font-weight:700;
            letter-spacing:2px;padding:3px 12px;border-radius:20px;margin-top:8px;position:relative;">
            STUDENT ID CARD
        </div>
    </div>

    <div style="text-align:center;margin-top:-30px;position:relative;z-index:2;">
        <div style="width:76px;height:76px;border-radius:50%;border:3px solid #2169ad;
            box-shadow:0 4px 12px rgba(33,105,173,.35);margin:0 auto;overflow:hidden;
            ...
            @if($photoUrl)
                ...
                <div style="display:none;width:100%;height:100%;align-items:center;
                    justify-content:center;font-size:22px;font-weight:700;color:#2169ad;
                    background:#dbeafe;">{{ $initials }}</div>
            @else
    ...
    <div style="text-align:center;padding:8px 18px 0;">
        <div style="font-size:14.5px;font-weight:800;color:#1e2937;line-height:1.2;">
            {{ $fullname }}
        </div>
        <div style="display:inline-block;background:#eff6ff;color:#2169ad;font-size:10px;
            font-weight:700;padding:3px 10px;border-radius:20px;margin-top:5px;letter-spacing:.5px;">
            {{ $student->admissionNo }}
        </div>
    </div>
    ...
    <div style="margin:10px 16px 0;background:#f8fafc;border-radius:8px;overflow:hidden;border:1px solid #e2e8f0;">
        <div style="display:flex;border-bottom:1px solid #e2e8f0;">
            <div style="width:95px;background:#eef2ff;padding:6px 10px;font-size:9px;
                font-weight:700;color:#4338ca;text-transform:uppercase;letter-spacing:.5px;">Class</div>
            <div style="flex:1;padding:6px 10px;font-size:10.5px;font-weight:600;color:#1e2937;">
                {{ $classArm ?: 'N/A' }}
            </div>
        </div>
        <div style="display:flex;border-bottom:1px solid #e2e8f0;">
            <div style="width:95px;background:#eef2ff;padding:6px 10px;font-size:9px;
                font-weight:700;color:#4338ca;text-transform:uppercase;letter-spacing:.5px;">Gender</div>
            <div style="flex:1;padding:6px 10px;font-size:10.5px;font-weight:600;color:#1e2937;">
                {{ $student->gender ?? 'N/A' }}
            </div>
        </div>
        <div style="display:flex;">
            <div style="width:95px;background:#eef2ff;padding:6px 10px;font-size:9px;
                font-weight:700;color:#4338ca;text-transform:uppercase;letter-spacing:.5px;">Valid Until</div>
            <div style="flex:1;padding:6px 10px;font-size:10.5px;font-weight:600;color:#1e2937;">
                {{ now()->addYear()->format('M Y') }}
            </div>
        </div>
    </div>
