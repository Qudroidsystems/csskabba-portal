...
.stu-card .check-wrap input[type=checkbox] {
...
.id-empty {
    text-align:center; padding:48px 20px; color:var(--id-muted);
}
...
.pag-btn:hover, .pag-btn.active {
    background:var(--id-accent); border-color:var(--id-accent); color:#fff;
}
.pag-btn:disabled { opacity:.4; cursor:not-allowed; }

/* ── Orientation toggle ── */
.orient-toggle .btn-outline-primary {
    font-size:12px; padding:6px 14px; border-radius:8px;
    border:1.5px solid var(--id-border); color:var(--id-primary);
}
.orient-toggle .btn-check:checked + .btn-outline-primary {
    background:var(--id-accent); border-color:var(--id-accent); color:#fff;
}
...
<div class="container-fluid">
    ...
    <div class="id-hero">
        ...
        <p>Generate, preview and download professional student ID cards with QR codes.</p>
    </div>
    ...
    <div class="id-panel" style="position:relative;">
        ...
        <div class="id-panel-header">
            ...
            <div class="d-flex gap-2">
                ...
                <button class="btn btn-sm btn-outline-secondary" id="clearSelectionBtn" style="display:none!important;" title="Clear Selection">
                    <i class="ri-close-line me-1"></i>Clear
                </button>
                <button class="btn btn-primary btn-sm" id="previewSelectedBtn" disabled>
                    <i class="fas fa-eye me-1"></i> Preview Selected (<span id="previewCount">0</span>)
                </button>
                @endcan
            </div>
        </div>
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                <div class="d-flex align-items-center gap-3">
                    <div class="form-check mb-0">
                        ...
                        <label class="form-check-label fw-semibold" for="selectAll" style="font-size:13px;">Select All</label>
                    </div>
...
$(document).ready(function () {
    ...
    /* ── State ── */
    let selectedIds   = new Set();   // persists across pages
    let currentPage   = 1;
    let totalPages    = 1;
    let lastMeta      = {};          // last pagination meta
    let studentsCache = [];          // current page data (for gender stats)
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.success) {
                    ...
                    showGridError('Failed to load students.');
                }
            },
            error: function (xhr) {
                console.error(xhr.responseText);
                showGridError('An error occurred while loading students. Please try again.');
            },
            complete: function () {
            ...
            <div class="col-6 col-sm-4 col-md-3 col-lg-2">
                <div class="stu-card${isChk ? ' selected' : ''}" data-id="${id}">
                    ...
                    <div class="text-center pt-4 pb-2 px-2">
                        ${photoUrl
                            ? `<img src="${photoUrl}" class="stu-photo" alt="${fullname}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
                    ...
                    <div class="pb-3 px-2 text-center">
                        ...
                        <div class="mt-1">
                            <span class="badge ${s.gender === 'Male' || s.gender === 'M' ? 'bg-primary' : 'bg-danger'} bg-opacity-10 text-${s.gender === 'Male' || s.gender === 'M' ? 'primary' : 'danger'}" style="font-size:10px;">
                                ${s.gender || 'N/A'}
                            </span>
                        </div>
                    </div>
    ...
    function updateStats(students, total) {
        ...
        const male   = students.filter(s => s.gender === 'Male' || s.gender === 'M').length;
        const female = students.filter(s => s.gender === 'Female' || s.gender === 'F').length;
        $('#statMale').text(male);
    ...
    function renderPagination(meta) {
        ...
        let html = '';
        // Prev
        html += `<button class="pag-btn me-1" id="prevPage" ${meta.current_page <= 1 ? 'disabled' : ''}>
        ...
        // Next
        html += `<button class="pag-btn" id="nextPage" ${meta.current_page >= meta.last_page ? 'disabled' : ''}>
    ...
    $(document).on('click', '.stu-card', function (e) {
        if ($(e.target).is('input[type=checkbox]')) return; // handled separately
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.success) {
                    ...
                    $('#previewBody').html('<div class="p-4 text-danger">Failed to generate preview.</div>');
                }
            },
            error: function (xhr) {
                console.error(xhr.responseText);
                $('#previewBody').html('<div class="p-4 text-danger">An error occurred. Check console for details.</div>');
            }
        });
    }
    ...
    function doDownload() {
        ...
        // Build form and POST it (so we get a file download, not AJAX)
        let form = $('<form>', { method: 'POST', action: '{{ route("student-id-cards.download") }}' });
        form.append($('<input>', { type: 'hidden', name: '_token',      value: CSRF }));
        form.append($('<input>', { type: 'hidden', name: 'orientation', value: orientation }));
