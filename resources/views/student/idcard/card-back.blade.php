{{--
    ...
    Usage: @include('student.idcard.card-back', ['student'=>$student, 'schoolInfo'=>$schoolInfo])
--}}
@php
    ...
    $barcode    = base64_encode(
        $barcodeGen->getBarcode($student->admissionNo, $barcodeGen::TYPE_CODE_128, 2, 50)
    );
@endphp
...
">
    ...
    ">
        <div style="position:absolute;top:-20px;left:-20px;width:80px;height:80px;border-radius:50%;background:rgba(255,255,255,.05);"></div>
        <div style="color:#fff;font-size:9px;font-weight:700;letter-spacing:2.5px;opacity:.8;">STUDENT IDENTITY CARD</div>
        <div style="color:#fff;font-weight:800;font-size:12px;margin-top:3px;letter-spacing:.3px;">
            {{ $schoolInfo?->school_name ?? 'School Name' }}
        </div>
    </div>

    {{-- ── MAGNETIC STRIP EFFECT ── --}}
    <div style="height:18px;background:#1a1a2e;margin-bottom:0;"></div>

    {{-- ── RULES / TERMS ── --}}
    <div style="padding:10px 16px 0;font-size:8.5px;color:#374151;line-height:1.6;">
        <div style="font-weight:700;font-size:9px;color:#1e3a5f;margin-bottom:4px;text-transform:uppercase;letter-spacing:.5px;">Terms &amp; Conditions</div>
    ...
    <div style="margin:10px 16px 0;background:#f0f4ff;border-radius:8px;padding:8px 10px;border-left:3px solid #2169ad;">
        <div style="font-size:8px;font-weight:700;color:#2169ad;text-transform:uppercase;letter-spacing:.8px;margin-bottom:4px;">School Contact</div>
        @if($address)
        ...
        @endif
        @if($website)
        <div style="font-size:8.5px;color:#2169ad;margin-bottom:2px;">&#127760; {{ $website }}</div>
        @endif
    </div>
    @endif
    ...
    <div style="margin:12px 16px 0;display:flex;gap:10px;">
        ...
        <div style="flex:1;text-align:center;">
            ...
            <div style="font-size:8px;color:#6b7280;margin-top:3px;letter-spacing:.3px;">Cardholder's Signature</div>
        </div>
        {{-- Authorised signature --}}
        <div style="flex:1;text-align:center;">
            <div style="height:36px;border-bottom:1.5px solid #1e3a5f;margin:0 6px;"></div>
            <div style="font-size:8px;color:#6b7280;margin-top:3px;letter-spacing:.3px;">Authorised Signature</div>
        </div>
    </div>
    ...
    <div style="text-align:center;margin-top:10px;padding:0 16px;">
        ...
        <div style="font-size:10px;font-weight:700;color:#1e3a5f;">{{ $schoolInfo?->school_name ?? 'School Administration' }}</div>
    ...
    <div style="text-align:center;padding:10px 0 6px;">
        ...
        <div style="font-size:7.5px;color:#94a3b8;margin-top:1px;">
            Valid Until: <strong>{{ $expiry }}</strong> &nbsp;|&nbsp; {{ $year }}
        </div>
    </div>
