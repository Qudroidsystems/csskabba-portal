...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ── Grade distribution - HORIZONTAL CHIPS ───────────────── */
        .grade-section {
            margin-bottom: 16px;
            background: #f8fafc;
            padding: 8px 12px;
        ...
        /* ── Table with student photo column ─────────────────────── */
        table.scores {
            width: 100%;
        ...
        .scores thead th { font-weight: 600; font-size: 9px; }
        .scores tbody tr:nth-child(even) { background: #f5f8fd; }
        .scores td.name-col { text-align: left; padding-left: 6px; }
...
@php
    ...
    // Using cum for pass/fail (or total based on your logic)
...
<div class="info-strip">
    <div class="info-item"><span class="info-label">Subject:</span> {{ $classInfo->subject ?? 'N/A' }} {{ $classInfo->subject_code ? '('.$classInfo->subject_code.')' : '' }}</div>
    <div class="info-item"><span class="info-label">Class:</span> {{ $classInfo->schoolclass ?? 'N/A' }} {{ $classInfo->arm ?? '' }}</div>
...
{{-- ── SUMMARY CARDS - HORIZONTAL LAYOUT (FLEX ROW) ────────────────── --}}
...
<div class="grade-section">
    ...
    <div class="grade-row">
        @foreach($gradeDist as $grade => $count)
            @php
                $col = $gradeColorMap[$grade] ?? '#6b7280';
                $pct = $total > 0 ? round($count/$total*100) : 0;
            @endphp
            <div class="grade-chip" style="color:{{ $col }};background:{{ $col }}10;border-color:{{ $col }}40;">
                {{ $grade }}: {{ $count }} ({{ $pct }}%)
            </div>
        @endforeach
    </div>
...
{{-- ── SCORES TABLE (WITH PHOTO COLUMN) ─────────────────────────────── --}}
<table class="scores">
    <thead>
        <tr>
            ...
            @foreach($assessments as $assessment)
                <th>{{ $assessment->name }}<br><span style="font-weight:400;font-size:8px;">({{ number_format($assessment->max_score, 0) }})</span></th>
            @endforeach
            <th style="background:#163275;">Total<br><span style="font-weight:400;font-size:8px;">({{ number_format($assessments->sum('max_score'), 0) }})</span></th>
            <th>BF</th>
            <th>Cum</th>
            <th>Grade</th>
            <th>Pos</th>
    ...
    <tbody>
        ...
        @empty
            <tr>
                <td colspan="{{ 6 + $assessments->count() + 5 }}" style="text-align:center;padding:12px;">
                    No students found.
                </td>
            </tr>
        @endforelse
    ...
    <tfoot>
        <tr style="background:#e8f0fe;font-weight:700;font-size:9px;">
            <td colspan="4" style="text-align:right;font-style:italic;">
                {{ $broadsheets->count() }} student(s) | Class Avg: {{ $avg }}
            </td>
            @foreach($assessments as $a)
                @php
                    $aSum = $broadsheets->sum(fn($b) => optional($b->assessmentScores->where('assessment_id',$a->id)->first())->score ?? 0);
                @endphp
