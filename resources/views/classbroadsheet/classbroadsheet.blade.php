...
.btn-back { background: rgba(255,255,255,.12); border: 1px solid rgba(255,255,255,.2); border-radius: 10px; padding: 8px 18px; color: #fff; font-size: 12px; font-weight: 600; text-decoration: none; display: inline-flex; align-items: center; gap: 8px; transition: all .2s; }
...
.autosave-chip { display: inline-flex; align-items: center; gap: 4px; font-size: 10px; font-weight: 700; padding: 2px 8px; border-radius: 20px; margin-top: 3px; transition: all .2s; }
...
.comment-status-dot { display: inline-flex; align-items: center; gap: 3px; font-size: 10px; font-weight: 700; padding: 1px 7px; border-radius: 20px; margin-top: 2px; }
...
.past-comment-item { transition: all 0.2s ease; animation: fadeInUp 0.3s ease; }
@keyframes fadeInUp { from { opacity:0; transform:translateY(10px); } to { opacity:1; transform:translateY(0); } }

.staff-avatar-img { width: 32px; height: 32px; border-radius: 50%; object-fit: cover; border: 2px solid; }
.staff-avatar-initials { width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 700; color: white; background: linear-gradient(135deg, var(--cb-teal), var(--cb-sky)); }
...
#cbCommentModal .modal-content { border-radius: var(--cb-radius); overflow: hidden; }
...
<div class="cb-hero">
    <div class="d-flex justify-content-between align-items-start flex-wrap gap-3">
        ...
        <a href="{{ route('myclass.index') }}" class="btn-back">
            <i class="ri-arrow-left-line"></i> Back to My Classes
        </a>
    </div>
</div>
...
<div class="row g-3 mb-4">
    ...
    <div class="col-6 col-md-3">
        <div class="cb-stat">
            ...
            <div class="stat-label">Avg % Obtained</div>
        </div>
    </div>
    <div class="col-6 col-md-3">
        <div class="cb-stat" id="topPerformerStat">
            ...
            <div class="stat-value text-warning" id="statTop">{{ $topPerformer ?? '—' }}</div>
            <div class="stat-label">Top Performer (% Obtained)</div>
            @if($topPerformerPicture)
            <div class="mt-2">
                <img src="{{ $topPerformerPicture }}" alt="Top Performer"
                     style="width: 40px; height: 40px; border-radius: 50%; border: 2px solid var(--cb-amber); object-fit: cover;">
            </div>
            @endif
        </div>
    </div>
...
<div class="col-toggle-panel">
    ...
    <p class="mt-2 mb-0" style="font-size:11px;color:var(--cb-muted);">
        <i class="ri-information-line me-1"></i> Click on any comment field to open the rich text editor with past comments history.
    </p>
</div>
...
<div class="cb-card">
    <div class="cb-card-header">
        <h5><i class="ri-table-alt-line" style="color:var(--cb-teal)"></i> Student Performance &amp; Comments <span class="badge" style="background:var(--cb-teal);color:#fff;font-size:11px;">{{ $students->count() }} Students</span></h5>
        <div class="cb-search" style="max-width:260px;"><i class="ri-search-line"></i><input type="text" id="searchInput" placeholder="Search students…"></div>
    </div>
    ...
    <form id="commentsForm">
                                ...
                                <div class="student-name-cell">
                                    @if($imgUrl)
                                        <div class="cb-avatar cb-avatar-trigger" data-img="{{ $imgUrl }}" data-name="{{ $fullName }}" data-adm="{{ $student->admissionNo }}" data-class="{{ $schoolclass ? $schoolclass->schoolclass . ' ' . $schoolclass->arm : '' }}" data-gender="{{ $student->gender ?? '' }}">
                                            <img src="{{ $imgUrl }}" alt="{{ $fullName }}" onerror="this.closest('.cb-avatar').classList.add('cb-avatar-initials'); this.remove(); this.closest('.cb-avatar').textContent='{{ $initials }}'">
                                        </div>
                                    @else
                                        <div class="cb-avatar cb-avatar-initials cb-avatar-trigger" data-img="" data-name="{{ $fullName }}" data-adm="{{ $student->admissionNo }}" data-class="{{ $schoolclass ? $schoolclass->schoolclass . ' ' . $schoolclass->arm : '' }}" data-gender="{{ $student->gender ?? '' }}" data-initials="{{ $initials }}">{{ $initials }}</div>
                                    @endif
                                    <div>
                            ...
                            @foreach ($subjects as $subject)
                                ...
                                <td class="cbcol-scores">
                                    <div class="score-dual">
                                        <div class="score-row score-row-term"><span class="score-lbl" style="color:#0891b2;">T</span><span class="{{ $tC }}">{{ $tScore ?: '—' }}</span>@if($tGrade !== '-')<span class="grade-badge g-{{ strtolower($tGrade) }}">{{ $tGrade }}</span>@endif</div>
                                        <div class="score-row score-row-cum"><span class="score-lbl" style="color:var(--cb-navy);">C</span><span class="{{ $cC }}">{{ $cScore ?: '—' }}</span>@if($cGrade !== '-')<span class="grade-badge g-{{ strtolower($cGrade) }}">{{ $cGrade }}</span>@endif</div>
                                    </div>
                                </td>
                            @endforeach
                            <td class="cbcol-summary analytics-cell">
                                <div class="analytics-row"><span class="analytics-lbl">Cum Total</span><span class="analytics-val">{{ $an['cum_total']??0 }}</span></div>
                                <div class="analytics-row"><span class="analytics-lbl">Obtainable</span><span class="analytics-val">{{ $an['total_obtainable']??0 }}</span></div>
                                <div class="analytics-row"><span class="analytics-lbl">% Obtained</span><span class="analytics-val {{ ($an['cum_percentage']??0)<50?'score-red':'score-green' }}">{{ $an['cum_percentage']??0 }}%</span></div>
                                <div class="pct-bar-wrap mt-1"><div class="pct-bar" style="width:{{ $an['cum_percentage']??0 }}%;background:{{ ($an['cum_percentage']??0)>=50?'var(--cb-green)':'var(--cb-rose)' }};"></div></div>
                                <div class="text-center mt-1"><button type="button" class="grade-trigger-btn" data-sid="{{ $sid }}" data-sname="{{ $fullName }}" title="View grade breakdown"><i class="ri-eye-line"></i></button></div>
                            </td>
        ...
        <div class="mobile-only" style="padding:16px;">
            @foreach ($students as $index => $student)
                @php
                    ...
                    $hasComment = $profile && !empty(trim($profile->classteachercomment ?? ''));
                @endphp
                <div class="cb-student-card cb-student-row {{ $hasComment ? 'card-has-comment' : 'card-no-comment' }}" data-student-id="{{ $sid }}" data-student-name="{{ $fullName }}" data-student-adm="{{ $student->admissionNo }}" data-student-img="{{ $imgUrl }}" data-student-analytics='@json($an)' data-searchkey="{{ strtolower($fullName . ' ' . $student->admissionNo) }}">
                    <div class="card-top">
                        @if($imgUrl)
                            <div class="cb-avatar cb-avatar-trigger" style="width:48px;height:48px;" data-img="{{ $imgUrl }}" data-name="{{ $fullName }}" data-adm="{{ $student->admissionNo }}" data-class="{{ $schoolclass ? $schoolclass->schoolclass . ' ' . $schoolclass->arm : '' }}" data-gender="{{ $student->gender ?? '' }}">
                                <img src="{{ $imgUrl }}" alt="{{ $fullName }}" style="width:100%;height:100%;object-fit:cover;" onerror="var p=this.closest('.cb-avatar');p.classList.add('cb-avatar-initials');p.textContent='{{ $initials }}'">
                            </div>
                        @else
                            <div class="cb-avatar cb-avatar-initials cb-avatar-trigger" style="width:48px;height:48px;font-size:16px;" data-img="" data-name="{{ $fullName }}" data-adm="{{ $student->admissionNo }}" data-class="{{ $schoolclass ? $schoolclass->schoolclass . ' ' . $schoolclass->arm : '' }}" data-gender="{{ $student->gender ?? '' }}" data-initials="{{ $initials }}">{{ $initials }}</div>
                        @endif
                        <div style="flex:1;">
                    ...
                    <div class="card-body-pad">
                        <div class="performance-strip">
                            ...
                            <div class="ps-grid">
                                <div class="ps-item"><div class="ps-lbl">Cum Total</div><div class="ps-val">{{ $an['cum_total']??0 }}</div></div>
                                <div class="ps-item"><div class="ps-lbl">Obtainable</div><div class="ps-val">{{ $an['total_obtainable']??0 }}</div></div>
                                <div class="ps-item"><div class="ps-lbl">% Obtained</div><div class="ps-val">{{ $an['cum_percentage']??0 }}%</div></div>
                                <div class="ps-item"><div class="ps-lbl">Cum Avg</div><div class="ps-val">{{ $an['cum_average']??0 }}</div></div>
                                <div class="ps-item"><div class="ps-lbl">Subjects</div><div class="ps-val">{{ $an['subject_count']??0 }}</div></div>
                            </div>
                        </div>
        ...
        <div class="save-bar">
            ...
            <div class="save-counter">
                <span class="save-counter-pill sc-pill-done" id="counterDone"><i class="ri-checkbox-circle-line"></i><span id="counterDoneNum">0</span> commented</span>
                <span class="save-counter-pill sc-pill-pending" id="counterPending"><i class="ri-time-line"></i><span id="counterPendingNum">0</span> pending</span>
            </div>
        ...
        <div class="modal-content" style="border-radius: var(--cb-radius); overflow: hidden;">
            ...
            <div class="modal-body" style="padding: 24px;">
                <div class="performance-strip" style="background: linear-gradient(135deg, var(--cb-navy), #1e5f74); border-radius: 12px; padding: 16px 20px; color: #fff; margin-bottom: 24px;">
                    <div style="font-size: 12px; font-weight: 600; opacity: 0.9; margin-bottom: 12px;"><i class="ri-bar-chart-line me-1"></i> Performance Summary</div>
                    <div class="ps-grid" style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 15px;">
                        <div class="ps-item" style="text-align: center; background: rgba(255,255,255,0.12); border-radius: 10px; padding: 10px;">
                            <div class="ps-lbl" style="font-size: 10px; opacity: 0.8; text-transform: uppercase; letter-spacing: 0.5px;">Cum Total</div>
                            <div class="ps-val" id="modalCumTotal" style="font-size: 20px; font-weight: 700; margin-top: 5px;">0</div>
                        </div>
                        <div class="ps-item" style="text-align: center; background: rgba(255,255,255,0.12); border-radius: 10px; padding: 10px;">
                            <div class="ps-lbl" style="font-size: 10px; opacity: 0.8; text-transform: uppercase; letter-spacing: 0.5px;">Obtainable</div>
                            <div class="ps-val" id="modalObtainable" style="font-size: 20px; font-weight: 700; margin-top: 5px;">0</div>
                        </div>
                        <div class="ps-item" style="text-align: center; background: rgba(255,255,255,0.12); border-radius: 10px; padding: 10px;">
                            <div class="ps-lbl" style="font-size: 10px; opacity: 0.8; text-transform: uppercase; letter-spacing: 0.5px;">% Obtained</div>
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div><h6 class="mb-0" id="modalCommentType" style="font-weight: 700; color: var(--cb-navy);"><i class="ri-chat-3-line me-1" style="color: var(--cb-teal);"></i> Teacher's Comment</h6><small class="text-muted" id="modalCommentHint">Click on any past comment to load it below</small></div>
...
(function () {
    ...
    function setChipState(sid, state, text) { ['autosave-' + sid, 'autosave-m-' + sid].forEach(function (id) { var chip = document.getElementById(id); if (!chip) return; chip.className = 'autosave-chip ' + state; chip.textContent = text || ''; }); }
    ...
    function autoSaveStudent(sid) {
        ...
        .then(function (data) {
            if (data.success) { setChipState(sid, 'ac-saved', '✓ Saved'); refreshCommentStatusForStudent(sid); setTimeout(function () { setChipState(sid, 'ac-idle', ''); }, 3000); }
            else { setChipState(sid, 'ac-err', '✗ Failed'); }
    ...
    function refreshCommentStatus() {
        ...
        document.querySelectorAll('.cb-student-row').forEach(function (row) { var sid = row.getAttribute('data-student-id'); if (!sid) return; if (getCanonical(sid, 'teacher').trim() !== '') done++; else pending++; refreshCommentStatusForStudent(sid); });
        var dNum = document.getElementById('counterDoneNum'); var pNum = document.getElementById('counterPendingNum');
        if (dNum) dNum.textContent = done; if (pNum) pNum.textContent = pending;
    }

    window.formatCommentText = function(type) {
        var ta = document.getElementById('modalTextarea'); if (!ta) return;
        var start = ta.selectionStart, end = ta.selectionEnd, text = ta.value, selectedText = text.substring(start, end);
        if (type === 'bold') { var formatted = '**' + (selectedText || 'bold text') + '**'; ta.value = text.substring(0, start) + formatted + text.substring(end); ta.focus(); ta.setSelectionRange(start + formatted.length, start + formatted.length); }
        else if (type === 'italic') { var formatted = '*' + (selectedText || 'italic text') + '*'; ta.value = text.substring(0, start) + formatted + text.substring(end); ta.focus(); ta.setSelectionRange(start + formatted.length, start + formatted.length); }
        else if (type === 'bullet') { if (selectedText) { var formatted = selectedText.split('\n').map(function(line) { return '• ' + line; }).join('\n'); ta.value = text.substring(0, start) + formatted + text.substring(end); } else { ta.value = text.substring(0, start) + '• ' + text.substring(end); } ta.focus(); }
        else if (type === 'number') { if (selectedText) { var formatted = selectedText.split('\n').map(function(line, idx) { return (idx + 1) + '. ' + line; }).join('\n'); ta.value = text.substring(0, start) + formatted + text.substring(end); } else { ta.value = text.substring(0, start) + '1. ' + text.substring(end); } ta.focus(); }
        else if (type === 'clear') { ta.value = ''; ta.focus(); }
    };
    ...
    function openCommentModal(sid, field, studentName, studentAdm, studentImg, analytics) {
        ...
        if (studentImg && studentImg !== 'null' && studentImg !== '') { avatarDiv.innerHTML = '<img src="' + studentImg + '" style="width:100%;height:100%;object-fit:cover;">'; }
        else { var initials = studentName.split(' ').map(function(n) { return n[0]; }).join('').substring(0, 2).toUpperCase(); avatarDiv.innerHTML = '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:28px;font-weight:700;">' + esc(initials) + '</div>'; }
        ...
        if (analytics.cum_percentage >= 70) cumPctEl.style.color = '#4ade80';
        else if (analytics.cum_percentage >= 50) cumPctEl.style.color = '#fbbf24';
        else cumPctEl.style.color = '#f87171';
    ...
    async function loadPastComments() {
        ...
        try {
            ...
            var response = await fetch(url, { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } });
            ...
            if (data.success && data.data && data.data.length > 0) {
                ...
                data.data.forEach(function(comment) {
                    var badgeColor = '', badgeIcon = '';
                    if (comment.comment_type === 'Teacher') { badgeColor = '#0ea5e9'; badgeIcon = 'ri-chat-quote-line'; }
                    else if (comment.comment_type === 'Guidance') { badgeColor = '#8b5cf6'; badgeIcon = 'ri-mental-health-line'; }
                    else if (comment.comment_type === 'Activities') { badgeColor = '#f59e0b'; badgeIcon = 'ri-football-line'; }
                    else { badgeColor = '#64748b'; badgeIcon = 'ri-chat-3-line'; }
                    var staffInitials = comment.staff_name.split(' ').map(function(n) { return n[0]; }).join('').substring(0, 2).toUpperCase();
                    commentsHtml += '<div class="past-comment-item" style="border-left: 4px solid ' + badgeColor + '; background: #ffffff; padding: 16px; margin-bottom: 14px; border-radius: 12px; cursor: pointer; box-shadow: 0 2px 6px rgba(0,0,0,0.05); transition: all 0.2s ease;" onclick="usePastComment(\'' + escapeHtml(comment.comment_text) + '\')" onmouseover="this.style.transform=\'translateX(4px)\'; this.style.boxShadow=\'0 6px 16px rgba(0,0,0,0.1)\';" onmouseout="this.style.transform=\'translateX(0)\'; this.style.boxShadow=\'0 2px 6px rgba(0,0,0,0.05)\';">';
                    commentsHtml += '<div class="d-flex justify-content-between align-items-start mb-3 flex-wrap gap-2"><div class="d-flex align-items-center gap-2"><span class="badge" style="background: ' + badgeColor + '; color: white; padding: 5px 12px; font-size: 11px;"><i class="' + badgeIcon + ' me-1" style="font-size: 11px;"></i> ' + comment.comment_type + '</span><span class="small text-muted"><i class="ri-calendar-line me-1"></i> ' + (comment.date || '—') + '</span></div></div>';
                    commentsHtml += '<div class="d-flex align-items-center gap-2 mb-3 p-2" style="background: #f8fafc; border-radius: 10px;"><div class="d-flex align-items-center justify-content-center" style="width: 32px; height: 32px; position: relative;"><span class="staff-avatar-initials">' + staffInitials + '</span></div><div><div class="small fw-semibold" style="color: #1e293b;"><i class="ri-user-line me-1" style="font-size: 10px;"></i> ' + escapeHtml(comment.staff_name) + '</div><div class="small text-muted" style="font-size: 10px;"><i class="ri-id-card-line me-1"></i> Staff ID: ' + (comment.staff_id || 'N/A') + '</div></div></div>';
                    commentsHtml += '<div class="mb-2"><span class="small fw-semibold" style="color: var(--cb-navy); background: #f1f5f9; padding: 3px 10px; border-radius: 15px; display: inline-flex; align-items: center; gap: 4px;"><i class="ri-calendar-event-line me-1" style="font-size: 11px;"></i> ' + escapeHtml(comment.session) + ' · ' + escapeHtml(comment.term) + '</span><span class="mx-1 text-muted">•</span><span class="small text-muted"><i class="ri-building-line me-1"></i> ' + escapeHtml(comment.class) + '</span></div>';
                    commentsHtml += '<div class="small" style="color: #334155; line-height: 1.6; background: #fefce8; padding: 10px 12px; border-radius: 10px; border-left: 3px solid ' + badgeColor + ';">' + escapeHtml(comment.comment_text.length > 150 ? comment.comment_text.substring(0, 150) + '...' : comment.comment_text) + '</div>';
                    commentsHtml += '<div class="mt-2 text-end"><span class="small text-primary" style="cursor: pointer;"><i class="ri-double-quotes-r"></i> Click to load this full comment</span></div></div>';
                });
                commentsHtml += '</div>';
                listEl.innerHTML = summaryHtml + commentsHtml;
                document.getElementById('pastCommentsPanel').style.display = 'block';
            } else {
                listEl.innerHTML = '<div class="text-center py-5"><i class="ri-inbox-line" style="font-size: 48px; color: #cbd5e1;"></i><p class="text-muted mt-2 mb-0">No past comments found for this student.</p><small class="text-muted">Comments from previous terms will appear here</small></div>';
                document.getElementById('pastCommentCount').textContent = '0';
                document.getElementById('pastCommentsPanel').style.display = 'block';
            }
        } catch (error) {
            console.error('Error loading past comments:', error);
            listEl.innerHTML = '<div class="text-center py-5 text-danger"><i class="ri-error-warning-line" style="font-size: 48px;"></i><p class="mt-2 mb-0">Failed to load past comments</p><small>Please refresh and try again</small></div>';
        }
    }

    function escapeHtml(text) { var div = document.createElement('div'); div.textContent = text; return div.innerHTML; }
    window.usePastComment = function(text) { var ta = document.getElementById('modalTextarea'); ta.value = text; ta.focus(); var notification = document.createElement('div'); notification.className = 'cb-toast cb-toast-success'; notification.innerHTML = '<i class="ri-checkbox-circle-fill"></i> Past comment loaded! You can edit it before saving.'; notification.style.position = 'fixed'; notification.style.bottom = '20px'; notification.style.right = '20px'; notification.style.zIndex = '10000'; document.body.appendChild(notification); setTimeout(function() { notification.remove(); }, 2000); };
    ...
    function attachModalTriggers() { document.querySelectorAll('.comment-input').forEach(function(input) { input.removeEventListener('focus', handleInputFocus); input.addEventListener('focus', handleInputFocus); }); }
    function handleInputFocus(e) {
        ...
        var analytics = {};
        if (analyticsRaw) { try { analytics = JSON.parse(analyticsRaw); } catch(e) { analytics = {}; } }
    ...
    function openGradePop(sid, name, triggerEl) {
        ...
        gpopBody.innerHTML = '<table class="gpop-table"><thead><tr><th>Subject</th><th style="color:#0891b2;">T.Score</th><th style="color:#0891b2;">T.Grade</th><th>C.Score</th><th>C.Grade</th><\/thead><tbody>' + (rows || '<tr><td colspan="5" class="text-center text-muted py-2">No grades<\/tr>') + '<\/tbody><\/table><div class="gpop-summary"><div class="gpop-sum-item"><div class="gpop-sum-lbl">Cum Total</div><div class="gpop-sum-val">' + (an.cum_total || 0) + '</div></div><div class="gpop-sum-item"><div class="gpop-sum-lbl">Obtainable</div><div class="gpop-sum-val">' + (an.total_obtainable || 0) + '</div></div><div class="gpop-sum-item"><div class="gpop-sum-lbl">% Obtained</div><div class="gpop-sum-val ' + ((an.cum_percentage || 0) < 50 ? 'score-red' : 'score-green') + '">' + (an.cum_percentage || 0) + '%</div></div></div>';
    ...
    function doSaveAll() {
        ...
        saveBtn.disabled = true; saveBtn.innerHTML = '<i class="spin ri-loader-4-line"></i> Saving…'; if (savingText) savingText.style.display = 'inline-flex';
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.addEventListener('click', function(e) {
            ...
            if (!trigger) return;
            var img = trigger.querySelector('img'), imgUrl = img ? img.src : null, name = trigger.getAttribute('data-name') || 'Student', adm = trigger.getAttribute('data-adm') || '', cls = trigger.getAttribute('data-class') || '', gender = trigger.getAttribute('data-gender') || '', initials = trigger.getAttribute('data-initials') || name.substring(0,2).toUpperCase();
            var zoomedImg = document.getElementById('cbZoomedImg'), zoomedName = document.getElementById('cbZoomedName'), zoomedMeta = document.getElementById('cbZoomedMeta');
            zoomedName.textContent = name;
            zoomedMeta.innerHTML = (adm ? '<i class="ri-id-card-line me-1"></i>' + esc(adm) : '') + (cls ? ' &nbsp;|&nbsp; <i class="ri-building-line me-1"></i>' + esc(cls) : '') + (gender ? ' &nbsp;|&nbsp; ' + esc(gender) : '');
            if (imgUrl && imgUrl !== 'null' && imgUrl !== '') { zoomedImg.src = imgUrl; }
            else { var canvas = document.createElement('canvas'); canvas.width = canvas.height = 400; var ctx = canvas.getContext('2d'); var grad = ctx.createLinearGradient(0, 0, 400, 400); grad.addColorStop(0, '#0d9488'); grad.addColorStop(1, '#0ea5e9'); ctx.fillStyle = grad; ctx.fillRect(0, 0, 400, 400); ctx.fillStyle = '#fff'; ctx.font = 'bold 150px "DM Sans",Arial,sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(initials.substring(0, 2).toUpperCase(), 200, 200); zoomedImg.src = canvas.toDataURL(); }
        ...
        (function() {
            ...
            if (vals.length) { var avgPct = Math.round(vals.reduce(function(s, d) { return s + (d.cum_percentage || 0); }, 0) / vals.length); var prEl = document.getElementById('statAvgPct'); if (prEl) prEl.textContent = avgPct + '%'; }
            var topAvg = -1, topName = '—', topPicture = null;
            document.querySelectorAll('.cb-student-row[data-student-id]').forEach(function(row) {
                ...
                var pct = SA[sid].cum_percentage || 0;
                if (pct > topAvg) { topAvg = pct; topName = row.getAttribute('data-student-name') || ''; topPicture = row.getAttribute('data-student-img') || null; }
            });
            var topEl = document.getElementById('statTop'); if (topEl) topEl.textContent = topName;
            if (topPicture && !document.querySelector('#topPerformerStat img')) { var statCard = document.getElementById('topPerformerStat'); if (statCard) { var imgDiv = document.createElement('div'); imgDiv.className = 'mt-2'; imgDiv.innerHTML = '<img src="' + topPicture + '" alt="Top Performer" style="width: 40px; height: 40px; border-radius: 50%; border: 2px solid var(--cb-amber); object-fit: cover;">'; statCard.appendChild(imgDiv); } }
        })();
        ...
        fieldSelectors.forEach(function(sel) { document.querySelectorAll(sel).forEach(function(inp) { inp.addEventListener('input', onInputChange); }); });
        attachModalTriggers();

        var loadBtn = document.getElementById('btnLoadPastComments'); if (loadBtn) loadBtn.addEventListener('click', loadPastComments);
        var modalSaveBtn = document.getElementById('modalSaveBtn'); if (modalSaveBtn) modalSaveBtn.addEventListener('click', saveCommentFromModal);
        var saveBtn = document.getElementById('saveBtn'); if (saveBtn) saveBtn.addEventListener('click', doSaveAll);
