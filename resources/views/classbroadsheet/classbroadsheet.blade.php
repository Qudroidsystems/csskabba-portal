...
.cb-breadcrumb .sep { font-size: 14px; color: rgba(255,255,255,.35); }
.cb-breadcrumb .current { color: #fff; font-weight: 600; }
...
.btn-back-inline:hover { color: #0f766e; }
...
<div class="cb-hero">
    ...
    <nav class="cb-breadcrumb">
        <a href="{{ route('myclass.index') }}">
            <i class="ri-home-4-line"></i> My Classes
        </a>
        <i class="ri-arrow-right-s-line sep"></i>
        <span class="current">
            {{ $schoolclass[0]->schoolclass ?? '' }} {{ $schoolclass[0]->arm ?? '' }}
            — Students
        </span>
    </nav>
    ...
    <div style="padding:20px 24px;border-top:1px solid var(--cb-border);background:#fafbfc;">
        <div class="row align-items-center">
            <div class="col-sm">
                ...
                {{-- ── Inline back link in footer ── --}}
                <a href="{{ route('myclass.index') }}" class="btn-back-inline">
                    <i class="ri-arrow-left-line"></i> Back to My Classes
                </a>
            </div>
    ...
    function buildTerminalReport(scores, assessments) {
        ...
        const rows = scores.map((s, i) => {
            ...
            const assessmentCells = asses.map(a => {
                ...
                return `<td class="${low ? 'sp-score-low' : ''}">${v !== null ? v : '—'}</td>`;
            }).join('');
            ...
            <table class="sp-report-table">
                <thead>
                    <tr>
                        ...
                        ${assessmentHeaders}
                        <th>Avg</th><th>Total</th><th>B/F</th><th>Cum</th>
                        <th>Grade</th><th>Cls Pos</th><th>Arm Pos</th><th>Cls Avg</th>
                    </tr>
                </thead>
                <tbody>${rows}</tbody>
            </table></div>
    ...
    function buildMockReport(scores) {
        ...
        c.innerHTML = `
            ...
            <table class="sp-report-table">
                <thead>
                    <tr>
                        <th>#</th><th style="text-align:left;">Subject</th>
                        <th>Exam</th><th>Total</th><th>Grade</th><th>Remark</th>
                        <th>Position</th><th>Cls Avg</th><th>Min</th><th>Max</th>
                    </tr>
                </thead>
                <tbody>${rows}</tbody>
            </table></div>
