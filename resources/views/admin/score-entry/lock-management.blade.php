...
/* ══════════════════════════════════════════════════════════════════════
   ...
   All cards (stat, filter, action-bar, table) share one layout root.
   No row/col wrappers outside stat cards — avoids gutter-offset drift.
   ══════════════════════════════════════════════════════════════════════ */
:root {
    --lm-primary : #1e3a5f;
    --lm-accent  : #2563eb;
...
/* ── Layout root — forces all children to same width ─────────────── */
.lm-page {
    display: flex;
...
.ib-lock:hover    { background:#fef9ec; border-color:#fbbf24; color:#d97706; box-shadow:0 2px 8px rgba(217,119,6,.2); }
.ib-unlock:hover  { background:#ecfdf5; border-color:#6ee7b7; color:#059669; box-shadow:0 2px 8px rgba(16,185,129,.2); }
.ib-disable:hover { background:#fef2f2; border-color:#fca5a5; color:#dc2626; box-shadow:0 2px 8px rgba(239,68,68,.2); }
.ib-enable:hover  { background:#eff6ff; border-color:#93c5fd; color:#2563eb; box-shadow:0 2px 8px rgba(37,99,235,.2); }
.ib-info:hover    { background:#f5f3ff; border-color:#c4b5fd; color:#7c3aed; box-shadow:0 2px 8px rgba(124,58,237,.2); }
...
@media (prefers-reduced-motion:reduce) {
    ...
    #scoresheetsTableBody tr.lm-row:hover { transform:none !important; }
}
...
<div class="container-fluid">
    ...
    <div class="lm-page">
        ...
        <div class="lm-stats">
            ...
            <div class="stat-card sc-open"     style="animation-delay:.08s">
                <div class="stat-ico"><i class="ri-lock-unlock-line"></i></div>
                <div class="stat-value" id="statOpen" style="color:var(--lm-success)">—</div>
                <div class="stat-label">Open</div>
            </div>
            <div class="stat-card sc-indiv"    style="animation-delay:.12s">
                <div class="stat-ico"><i class="ri-lock-line"></i></div>
                <div class="stat-value" id="statIndividual" style="color:var(--lm-warning)">—</div>
                <div class="stat-label">Individually Locked</div>
            </div>
            <div class="stat-card sc-global"   style="animation-delay:.16s">
                <div class="stat-ico"><i class="ri-global-line"></i></div>
                <div class="stat-value" id="statGlobal" style="color:var(--lm-danger)">—</div>
                <div class="stat-label">Globally Locked</div>
            </div>
            <div class="stat-card sc-disabled" style="animation-delay:.20s">
                <div class="stat-ico"><i class="ri-ban-line"></i></div>
                <div class="stat-value" id="statDisabled" style="color:var(--lm-muted)">—</div>
        ...
        <div class="lm-filter">
            <div class="row g-2 align-items-end">
                ...
                <div class="col-md-1">
                    <button class="btn btn-primary w-100" id="applyFiltersBtn" style="height:38px;" title="Apply filters">
        ...
        <div class="lm-table-card">
            <div class="tc-header">
                <h5><i class="ri-list-check"></i>Scoresheets</h5>
                <span class="tc-badge" id="recordCount">—</span>
            </div>
            <div style="overflow-x:auto; -webkit-overflow-scrolling:touch;">
                <table id="scoresheetsTable">
                    <thead>
                        <tr>
                            <th style="width:42px;padding-left:16px;">
                    ...
                    <tbody id="scoresheetsTableBody">
                        <tr class="lm-state-row">
                            <td colspan="7">
                                <i class="ri-loader-4-line lm-spin lm-state-icon"></i>
                                Loading scoresheets…
                            </td>
                        </tr>
                    </tbody>
            ...
            <div class="tc-footer">
                <span><i class="ri-information-line me-1 text-info"></i>
                    Use row icons for single-row actions · action bar for bulk operations.
                </span>
                <span id="footerCount" class="fw-semibold" style="color:var(--lm-primary);">— records</span>
            </div>
        </div>
...
async function fetchAndRender(buildFilters) {
    ...
    try {
        ...
        if (!result.success) {
            showError(result.message || 'Failed to load scoresheets.');
            toast(result.message || 'Failed to load.', 'error');
            return;
        }

        allData = result.data || [];

        if (buildFilters && result.filters && !filtersReady) {
            buildFilterDropdowns(result.filters);
            filtersReady = true;
        }
        ...
        toast('Network error loading scoresheets.', 'error');
    }
}
    ...
    const rows = Array.from({length:6}).map(() => `
        <tr class="lm-state-row" style="opacity:1;transform:none;">
            <td colspan="7" style="padding:0;">
                <div style="display:flex;align-items:center;gap:14px;padding:13px 16px;border-bottom:1px solid var(--lm-border);">
...
function showError(msg) {
    document.getElementById('scoresheetsTableBody').innerHTML = `
        <tr class="lm-state-row"><td colspan="7">
            <i class="ri-error-warning-line lm-state-icon" style="color:#ef4444;opacity:1;"></i>
            ${esc(msg)}
        </td></tr>`;
}
        ...
        tr.innerHTML = `
            ...
            <td>
                <div style="display:flex;align-items:center;gap:10px;">
                    <div class="t-avatar">${esc(ini)}</div>
                    <div style="min-width:0;">
                        <div style="font-weight:600;font-size:13px;color:#1e293b;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:220px;">
                            ${esc(sheet.teacher_name)}
                        </div>
                        <div style="font-size:11.5px;color:#64748b;margin-top:1px;">
                            ${esc(sheet.subject_name)}<span class="subj-code">${esc(sheet.subject_code)}</span>
                        </div>
                        ${sheet.individually_locked_count > 0 ? `
                        <div style="margin-top:5px;min-width:120px;">
                            <div style="font-size:10px;color:var(--lm-muted);margin-bottom:3px;">
                                <i class="ri-lock-line"></i> ${sheet.individually_locked_count}/${sheet.total_students} locked
                            </div>
            ...
            <td>
                <div class="row-actions">
                    <button class="ib ib-lock"    title="Lock individual"    onclick="quickAction(${sheet.subjectclass_id},'lock_individual')"><i class="ri-lock-line"></i></button>
...
function staggerRows() {
    ...
    const io   = new IntersectionObserver(entries => {
        entries.forEach(e => {
            ...
            setTimeout(() => e.target.classList.add('visible'), Math.min(idx * 35, 14 * 35) + 40);
            io.unobserve(e.target);
        });
    }, { threshold:.05, rootMargin:'0px 0px -15px 0px' });
...
function countUp(id, target) {
    ...
    const iv = setInterval(() => {
        i++;
        el.textContent = i >= steps ? target : Math.round(from + (target - from) * (i/steps));
...
function onSelectAll() {
    ...
    document.querySelectorAll('#scoresheetsTableBody .row-cb').forEach(cb => {
        cb.checked = checked;
...
function toast(msg, type='info') {
    ...
    el.innerHTML = `${icons[type]||icons.info}<span class="tm">${esc(msg)}</span>
        <button class="tc" onclick="this.closest('.lm-toast').remove()"><i class="ri-close-line"></i></button>`;
    document.getElementById('lmToasts').appendChild(el);
    setTimeout(()=>{ el.style.transition='opacity .3s'; el.style.opacity='0'; setTimeout(()=>el.remove(),320); }, 5000);
}

function spinRefresh() {
    const ico = document.querySelector('#refreshBtn i');
    if (!ico) return;
    ico.classList.add('lm-spin');
    setTimeout(()=>ico.classList.remove('lm-spin'), 900);
}
</script>
@endsection
