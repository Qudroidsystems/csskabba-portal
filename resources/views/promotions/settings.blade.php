...
function buildRuleHTML(rule, idx) {
    ...
    const groupingOpts = [['grouped', isSenior ? 'Grouped (A=A1, B=B2+B3…)' : 'Grouped (A, B, C…)'], ['exact', isSenior ? 'Exact (A1, B2, B3 separately)' : 'Exact (A, B, C separately)']].map(([v, l]) => `<option value="${v}" ${grouping === v ? 'selected' : ''}>${l}</option>`).join('');

    const subjects = rule.compulsory_section?.subjects ?? [];
    const compSubjRowsHtml = !subjects.length ? `<div class="text-muted small p-3"><i class="ri-information-line me-1"></i>No compulsory subjects assigned to this class.</div>` :
        subjects.map((subj, si) => `<div class="comp-subj-row"><div><span class="subj-name">${escH(subj.subject_name)}</span>${subj.subject_code ? `<span class="subj-code ms-1">(${escH(subj.subject_code)})</span>` : ''}${subj.default_min_grade ? `<span class="default-badge ms-2"><i class="ri-information-line"></i> default: ${subj.default_min_grade}</span>` : ''}${subj.override && subj.min_grade && subj.min_grade !== subj.default_min_grade ? `<span class="badge bg-warning text-dark ms-1">overridden</span>` : ''}</div><select class="grade-sel comp-subj-grade-sel" data-idx="${idx}" data-si="${si}">${['', ...gradeScale].map(g => `<option value="${g}" ${subj.min_grade === g ? 'selected' : ''}>${g === '' ? '— Any (default pass/fail)' : g}</option>`).join('')}</select></div>`).join('');
    ...
    return `<div class="rule-card" data-rule-idx="${idx}"><div class="rule-card-header"><span class="rule-num-badge">Rule ${idx + 1}</span><span class="badge bg-${statusBadgeClass}" id="statusBadge_${idx}"><i class="${selSt.icon} me-1"></i>${selSt.label}</span><input type="text" class="form-control form-control-sm rule-name-input" data-idx="${idx}" value="${escH(rule.rule_name)}" placeholder="Rule name"><div class="d-flex gap-1 align-items-center ms-auto"><span class="text-muted small me-1">Priority:</span><input type="number" class="form-control form-control-sm priority-input" data-idx="${idx}" value="${rule.priority ?? idx + 1}" min="1" style="width:65px;"><button class="btn btn-sm btn-outline-secondary move-up-btn" data-idx="${idx}" title="Move up"><i class="ri-arrow-up-line"></i></button><button class="btn btn-sm btn-outline-secondary move-down-btn" data-idx="${idx}" title="Move down"><i class="ri-arrow-down-line"></i></button><button class="btn btn-sm btn-outline-danger remove-rule-btn" data-idx="${idx}" title="Remove rule"><i class="ri-delete-bin-line"></i></button></div></div><div class="rule-card-body"><div class="mb-3"><label class="fw-semibold small d-block mb-2"><i class="ri-award-line me-1"></i>Promotion Outcome</label><div class="label-selector">${labelPills}</div></div><div class="mb-3"><label class="fw-semibold small"><i class="ri-git-branch-line me-1"></i>Grade Grouping <small class="text-muted fw-normal ms-1">(changing this resets count conditions)</small></label><select class="form-select form-select-sm grouping-sel mt-1" data-idx="${idx}" style="max-width:300px;">${groupingOpts}</select></div><div class="rule-section mb-3"><div class="rule-section-header"><span><i class="ri-star-fill text-warning me-2"></i>Section 1 — Compulsory Subjects <small class="text-muted fw-normal ms-2">(${subjects.length} subjects)</small></span></div><div class="rule-section-body"><div class="mb-3"><div class="fw-semibold small mb-2"><i class="ri-shield-check-line text-warning me-1"></i>Per-subject minimum grade</div><div id="compSubjRows_${idx}">${compSubjRowsHtml}</div></div><hr class="my-2"><div><div class="d-flex align-items-center justify-content-between mb-2"><div class="fw-semibold small"><i class="ri-bar-chart-line me-1"></i>Grade count conditions across compulsory subjects</div><div class="d-flex gap-1 align-items-center"><select class="form-select form-select-sm" id="addCompGrade_${idx}" style="width:80px;">${gradeOptsComp}</select><button class="btn btn-outline-primary btn-sm add-comp-cond-btn" data-idx="${idx}"><i class="ri-add-line"></i> Add</button></div></div><div id="compCondRows_${idx}">${compCondRows || '<div class="text-muted small ps-1 py-1">No count conditions — click Add above.</div>'}</div></div></div></div><div class="rule-section mb-3"><div class="rule-section-header"><span><i class="ri-book-open-line text-primary me-2"></i>Section 2 — Other / All Subjects Grade Count <small class="text-muted fw-normal ms-2">(${otherCount} other subjects)</small></span></div><div class="rule-section-body"><div class="d-flex align-items-center justify-content-between mb-2"><div class="fw-semibold small"><i class="ri-bar-chart-line me-1"></i>Grade count conditions</div><div class="d-flex gap-1 align-items-center"><select class="form-select form-select-sm" id="addOtherGrade_${idx}" style="width:80px;">${gradeOptsOther}</select><button class="btn btn-outline-primary btn-sm add-other-cond-btn" data-idx="${idx}"><i class="ri-add-line"></i> Add</button></div></div><div id="otherCondRows_${idx}">${otherCondRows || '<div class="text-muted small ps-1 py-1">No count conditions — click Add above.</div>'}</div></div></div><div class="avg-box"><div class="form-check form-switch mb-2"><input class="form-check-input avg-toggle-cb" type="checkbox" role="switch" id="avgCb_${idx}" data-idx="${idx}" ${avg.enabled ? 'checked' : ''}><label class="form-check-label fw-semibold small" for="avgCb_${idx}"><i class="ri-percent-line text-info me-1"></i>Section 3 — Minimum Average Condition (optional)</label></div><div id="avgFields_${idx}" style="${avg.enabled ? '' : 'opacity:.4;pointer-events:none;'}"><div class="row g-2"><div class="col-md-4"><label class="form-label small fw-semibold mb-1">Min Average (%)</label><input type="number" class="form-control form-control-sm avg-min-inp" data-idx="${idx}" min="0" max="100" step="0.5" value="${avg.min_average ?? (classPassAvg ?? 50)}"></div><div class="col-md-4"><label class="form-label small fw-semibold mb-1">Logic with Sections 1+2</label><select class="form-select form-select-sm avg-logic-sel" data-idx="${idx}"><option value="AND" ${avg.logic === 'AND' ? 'selected' : ''}>AND (all sections must pass)</option><option value="OR" ${avg.logic === 'OR' ? 'selected' : ''}>OR (average alone qualifies)</option></select></div></div></div></div></div></div>`;
}

function rerenderRules() {
    const container = document.getElementById('rulesContainer');
    const noMsg = document.getElementById('noRulesMsg');
    if (!promotionRules.length) { if (container) { container.innerHTML = ''; if (noMsg) { container.appendChild(noMsg); noMsg.style.display = 'block'; } } return; }
    if (noMsg) noMsg.style.display = 'none';
    if (container) { container.innerHTML = ''; promotionRules.forEach((rule, idx) => { const div = document.createElement('div'); div.innerHTML = buildRuleHTML(rule, idx); container.appendChild(div.firstElementChild); }); }
}
...
function handleContainerClick(e) {
    ...
    if (addCompBtn) {
        ...
        if (gradeSelect && gradeSelect.value) {
            ...
            rerenderRules();
        } else if (gradeSelect && !gradeSelect.value) Swal.fire('Warning', 'Please select a grade first.', 'warning');
        return;
    }
...
function handleContainerChange(e) {
    ...
    if (condOpSel) { const ri = parseInt(condOpSel.dataset.rule), sec = condOpSel.dataset.sec, ci = parseInt(condOpSel.dataset.ci); const conds = _getConds(ri, sec); if (conds[ci]) conds[ci].operator = condOpSel.value; return; }
    const condScopeSel = e.target.closest('.cond-scope-sel');
    if (condScopeSel) { const ri = parseInt(condScopeSel.dataset.rule), sec = condScopeSel.dataset.sec, ci = parseInt(condScopeSel.dataset.ci); const conds = _getConds(ri, sec); if (conds[ci]) conds[ci].scope = condScopeSel.value; return; }
    ...
    if (avgLogicSel) { const idx = parseInt(avgLogicSel.dataset.idx); if (!promotionRules[idx].average_condition) promotionRules[idx].average_condition = {}; promotionRules[idx].average_condition.logic = avgLogicSel.value; return; }
}
...
function handleContainerInput(e) {
    ...
    if (condCountInp) { const ri = parseInt(condCountInp.dataset.rule), sec = condCountInp.dataset.sec, ci = parseInt(condCountInp.dataset.ci); const conds = _getConds(ri, sec); if (conds[ci]) conds[ci].count = parseInt(condCountInp.value); return; }
    const avgMinInp = e.target.closest('.avg-min-inp');
    if (avgMinInp) { const idx = parseInt(avgMinInp.dataset.idx); if (!promotionRules[idx].average_condition) promotionRules[idx].average_condition = {}; promotionRules[idx].average_condition.min_average = parseFloat(avgMinInp.value); return; }
}
...
async function refreshClassInfo() {
    ...
    try {
        ...
        if (data.success) {
            ...
            if (classPassAvg) { document.getElementById('promotion_pass_average').value = classPassAvg; document.getElementById('avg_slider').value = classPassAvg; }
            ...
            if (promotionRules.length > 0) promotionRules = promotionRules.map(rule => { if (!rule.compulsory_section) rule.compulsory_section = { subjects: [], count_conditions: [] }; if (!rule.other_section) rule.other_section = { count_conditions: [] }; const existing = (rule.compulsory_section.subjects).reduce((m, s) => { m[String(s.subject_id)] = s; return m; }, {}); rule.compulsory_section.subjects = compulsorySubjects.map(cs => ({ subject_id: cs.subject_id, subject_name: cs.subject_name, subject_code: cs.subject_code, default_min_grade: cs.default_min_grade ?? '', min_grade: existing[String(cs.subject_id)]?.min_grade ?? cs.default_min_grade ?? '', override: !!(existing[String(cs.subject_id)]?.min_grade) })); return rule; });
            rerenderRules();
        } else {
    ...
    } catch (err) { loadEl.style.display = 'none'; summaryEl.innerHTML = `<div class="alert alert-danger py-2 mb-0"><i class="ri-error-warning-line me-1"></i>Error: ${err.message}</div>`; summaryEl.style.display = 'block'; }
}
...
document.getElementById('saveSettingBtn')?.addEventListener('click', async function() {
    ...
    for (const [i, rule] of promotionRules.entries()) {
        ...
        if (!hasCompSubjGrades && !hasCompConds && !hasOtherConds && !hasAvg) { Swal.fire('Validation', `Rule ${i + 1} has no conditions.`, 'warning'); return; }
    }
...
// ── Edit/Delete ─────────────────────────────────────────────────────────────────
function bindEditButtons() { document.querySelectorAll('.edit-setting').forEach(btn => { btn.removeEventListener('click', handleEditClick); btn.addEventListener('click', handleEditClick); }); }
...
function bindDeleteButtons() { document.querySelectorAll('.delete-setting').forEach(btn => { btn.removeEventListener('click', handleDeleteClick); btn.addEventListener('click', handleDeleteClick); }); }
...
document.getElementById('rule_logic')?.addEventListener('change', function() { document.getElementById('globalAvgSection').style.display = ['average_only', 'both'].includes(this.value) ? 'block' : 'none'; });
...
document.getElementById('addRuleBtn')?.addEventListener('click', () => {
    promotionRules.push({ rule_name: '', status_label: 'promoted', priority: promotionRules.length + 1, grade_grouping: 'grouped', compulsory_section: { subjects: compulsorySubjects.map(cs => ({ subject_id: cs.subject_id, subject_name: cs.subject_name, subject_code: cs.subject_code, default_min_grade: cs.default_min_grade ?? '', min_grade: cs.default_min_grade ?? '', override: false })), count_conditions: [] }, other_section: { count_conditions: [] }, average_condition: { enabled: false, min_average: classPassAvg ?? 50, logic: 'AND' } });
    rerenderRules();
});
document.getElementById('templateSelect')?.addEventListener('change', function() { document.getElementById('loadTemplateBtn').disabled = !this.value; document.getElementById('template_id_input').value = this.value; });
document.getElementById('loadTemplateBtn')?.addEventListener('click', async function() {
    ...
    try {
        ...
        const res = await fetch(url);
        const data = await res.json();
        if (data.success) { promotionRules = data.merged_rules ?? []; status.textContent = `✓ ${data.template.name} loaded (${promotionRules.length} rules)`; status.style.color = '#16a34a'; rerenderRules(); }
        else { status.textContent = '✗ ' + (data.message || 'Failed'); status.style.color = '#dc2626'; }
