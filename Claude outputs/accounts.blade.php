                        ...
                        @foreach($accounts[$type] as $a)
                            <tr class="{{ $a->is_active ? '' : 'text-muted' }}">
                                ...
                                <td class="text-end text-nowrap"><a href="{{ route('accounting.ledger', ['account' => $a->id]) }}" class="action-btn btn-open" title="Ledger"><i class="ri-file-list-3-line"></i></a>
                                    @can('Manage chart of accounts')<button type="button" class="action-btn btn-open" title="Edit" onclick='editAcc(@json($a->only(["id","account_code","account_name","account_type","bank_name","bank_account_no","description","is_bank_account","is_active"])))'><i class="ri-edit-line"></i></button>@endcan</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table></div>
